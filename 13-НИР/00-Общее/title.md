...
Рисунки и схемы: архитектура модуля Elevation Mapping, pipeline фильтрации облака точек LiDAR, результаты ground segmentation, цифровая модель высот (DEM), карты gradient и roughness поверхности, функция стоимости пути (traversability cost map), схема terrain-aware планирования маршрута, адаптация походки по типам местности, результаты тестирования в симуляции Gazebo.

Таблицы: метрики производительности модуля (частота обновления карты, время обработки облака точек), параметры походки для различных классов местности (safe, medium, unsafe).
