...
Visibility cleanup — алгоритм на CUDA, корректирующий карту высот на основе анализа видимости (описание приведено в Главе 2, раздел 2.4.4). Эффект: динамические объекты автоматически удаляются через несколько кадров; шумовые измерения не создают ложных объектов [5].
