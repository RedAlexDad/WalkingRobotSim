...
RAM-фильтр (Ramped Adaptive Maximum height) реализует адаптивный порог высоты, зависящий от расстояния до точки. На близких расстояниях (d < b) порог равен минимальному значению c; на дальних (d > b) порог линейно растёт с коэффициентом a. Для a = 0,3, b = 1,0, c = 0,2: на 0,3 м порог 0,2 м; на 2,0 м — 0,5 м; на 10,0 м — 2,9 м. Это позволяет на близких расстояниях отбрасывать потолки и нависающие объекты, а на дальних — учитывать естественный рельеф [4].

### 3.6.3 Visibility Cleanup и фильтрация Махаланобиса

Visibility cleanup — алгоритм на CUDA, корректирующий карту высот на основе анализа видимости. Для каждой ячейки карты определяется, находится ли она на прямой видимости между сенсором и точкой измерения. Если ячейка на линии видимости, её уверенность уменьшается на cleanup_step (0,1). Эффект: динамические объекты автоматически удаляются через несколько кадров; шумовые измерения не создают ложных объектов [5].

Для дополнительной фильтрации используется критерий Махаланобиса: если расстояние между измеренной и ожидаемой высотой превышает mahalanobis_thresh = 2,0, точка отбрасывается (отклонение более чем на 2σ).
