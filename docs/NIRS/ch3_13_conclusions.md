...
- Интегрировано GPU-ускоренное построение карты высот с использованием пакета elevation_mapping_cupy в Docker-окружение проекта, обеспечена корректная работа CuPy-ядер на GPU NVIDIA GTX 1650 Ti.
- Организована передача данных трёхмерного LiDAR от симулятора к модулю карты высот через кросс-контейнерную связь с Cyclone DDS, выполнена настройка QoS и TF-трансформаций.
- Выполнена сегментация облака точек на классы ground и non-ground с использованием Ground Plane Fitting с 3 итерациями RANSAC, воксельным понижением и height_margin post-filter.
- Построена и обновляется цифровая модель высот (DEM) с разрешением 0,1 м и размером 20×20 м, вычислены gradient поверхности (PCA, окно 3×3) и roughness (RMSE, окно 5×5).
- Разработана функция стоимости пути (traversability cost), учитывающая уклон, шероховатость и перепады высот для terrain-aware планирования маршрута с весами 0,5/0,3/0,2.
- Реализована адаптация параметров походки (высота шага 0,04–0,15 м, частота 1,0–2,0 Гц, скорость 0,15–0,5 м/с) в зависимости от traversability под опорами робота с экспоненциальным сглаживанием.
- Разработан мост GridMap → OccupancyGrid для интеграции traversability с Nav2, выполнена настройка QoS (TRANSIENT_LOCAL) и задокументирована проблема frame shift.
- Разработан ground truth bridge для диагностики одометрии и анализа дрейфа leg odometry с публикацией GT-данных от Gazebo.
- Реализован stall detection для предотвращения накопления дрейфа при блокировке движения робота с параметризуемыми порогами угловой скорости.
- Создана CPU-версия Docker-образа (Dockerfile.cpu) для отладки без GPU passthrough с производительностью ~5 Гц.
- Выполнено тестирование и валидация разработанного модуля в симуляции Gazebo с пятью сценариями рельефа. Целевые показатели (RMSE < 0,05 м, FPS ≥ 10 Гц, задержка обработки < 100 мс) достигнуты.
