...
## Сравнение производительности Old vs New (Script) Python модули

### Результаты бенчмарка (5000 итераций)

**Дата: 2026-04-07**

**Метод:** Python скрипт запускает old и new модули отдельно, сравнивает время выполнения.

| Функция | Old (ms) | New (ms) | Разница | x быстрее |
|---------|----------|----------|---------|-----------|
| FK | 0.4112 | 0.4106 | +0.0006 | 1.00x |
| IK | 0.1375 | 0.0095 | +0.1280 | **14.51x** |
| homog_transform_inverse | 0.0202 | 0.0197 | +0.0004 | 1.02x |
| local_positions | 0.1250 | 0.0839 | +0.0411 | 1.49x |
| rotxyz | 0.0099 | 0.0096 | +0.0003 | 1.03x |
| **ИТОГО** | **0.7038** | **0.5333** | **+0.1704** | **1.32x быстрее** |

### Ключевые улучшения

1. **InverseKinematics: 14.51x быстрее** (0.14ms → 0.009ms)
   - Основная причина: использование numpy array вместо Python list
   - Оптимизированная функция `compute_all_joint_angles` с поддержкой формата (3,4)

2. **local_positions: 1.49x быстрее** (0.13ms → 0.08ms)
   - Исправлен алгоритм: теперь использует тот же `rotxyz(π/2, -π/2, 0)` что и C++
   - Поддержка numpy matrix operations вместо list-based

3. **Общее ускорение: 1.32x** (0.70ms → 0.53ms)

### Что было исправлено

1. **joint_angles.py**: Функция `compute_all_joint_angles` теперь поддерживает формат (3,4) вместо (4,3), что соответствует C++ и новому local_positions
2. **local_positions.py**: Использует правильную матрицу вращения `rotxyz(π/2, -π/2, 0)` как в C++

### Тесты корректности

Все 5 тестов корректности проходят:
- test_all_modules.py ✅
- test_old_vs_new.py ✅
- test_step_trot.py ✅
...
Tolerance: 1e-6 (достаточно строгий для robotics)
...
## Сравнение переключения режимов gait (Trot, Crawl, Rest, Stand)
