...
| 4 | «Эффективность — что это конкретно?» | Нужны измеримые критерии | Задать критерии (скорость, точность восприятия) |
| 5 | «Научно доказать, что тестируешь» | Нужен корректный метод оценки | Продумать методику без профтестирования |
| 6 | Три вопроса: объект, где анализ, характеристики | Каркас постановки задачи | Ответить письменно на все три |
| 7 | «Посоветуйся с Антоном» | Привлечь научрука | Согласовать тему с Каневым А.И. |

**Главная идея одной фразой:** взять не «состояние робота вообще», а
конкретный **экран оператора (информационную модель)** и доказать
эргономически, что выбранный состав и форма отображения информации
обеспечивают измеримо лучшую **эффективность восприятия** по сравнению
с альтернативой.
...
### 7.3. Измеримые критерии

| Критерий | Что измеряет | Как измерить | Единица |
|---|---|---|---|
| Время считывания | скорость восприятия | замер по протоколу | с / мс |
| Точность считывания | безошибочность | число ошибок | % |
| Время обнаружения аномалии | реакция на отклонение | замер на записи | с |
| Нагрузка на внимание | объём одновременно | число параметров | шт. |
| Пропуск события | надёжность | доля пропусков | % |
| Число действий до решения | оперативность | подсчёт | шт. |

### 7.4. Схема связи «эффективность → критерий»

```mermaid
graph TB
    EFF["Эффективность<br/>оператора"] --> C1["Скорость<br/>восприятия"]
    EFF --> C2["Точность<br/>восприятия"]
    EFF --> C3["Надёжность<br/>(нет пропусков)"]
    EFF --> C4["Нагрузка<br/>на внимание"]
    C1 --> M1["Время<br/>считывания"]
    C2 --> M2["Число<br/>ошибок"]
    C3 --> M3["Время<br/>обнаружения"]
    C4 --> M4["Число<br/>параметров"]
```

### 7.5. Рабочее определение для КР

**Эффективность информационной модели** — это способность оператора
за минимальное время и с минимальным числом ошибок воспринимать
критичные параметры состояния робота и обнаруживать аномалии.

Это определение:
- относится к человеку (эргономика);
- измеримо (время, ошибки);
...
**Где здесь анализ?** Что именно сравнивается, оценивается или
исследуется. Ответ: сравнение вариантов представления информации по
критериям эффективности восприятия.

### 8.3. Вопрос 3 — характеристики

**Какие конкретные характеристики?** Не «всё состояние», а измеримый
набор. Ответ: время и точность считывания, время обнаружения аномалии,
...
|---|---|---|---|
| Метод экспертных оценок | группа оценивает варианты | высокая | нужна группа, согласованность |
| Анализ литературы | опора на известные нормы | высокая | нет своего эксперимента |
| Аналитическая оценка | расчёт по критериям | средняя | нужны веса критериев |
| Замер на записи | оценка по логам | средняя | без живого оператора |
| Пилотный эксперимент | малая выборка | низкая | риск некорректности |

### 9.3. Рекомендуемый подход

Комбинация: **аналитическая оценка на основе норм эргономики** +
**метод экспертных оценок** (небольшая группа) + **иллюстрация на
реальных данных** (записи телеметрии из симуляции). Это не полноценный
психофизический эксперимент, но научно обоснованная оценка, не
требующая профессиональных операторов.

### 9.4. Схема доказательства

```mermaid
graph LR
    L["Нормы<br/>эргономики"] --> J["Обоснованная<br/>оценка ИМ"]
    X["Экспертные<br/>оценки"] --> J
    D["Реальные данные<br/>(логи симуляции)"] --> J
    J --> C["Вывод:<br/>какой вариант лучше<br/>и почему"]
```

### 9.5. Честная оценка риска

Это по-прежнему самое слабое место. Горячкин может спросить: «Насколько
репрезентативна ваша группа?» Поэтому важно заранее подготовить
обоснование: почему выбранные методы достаточны для учебной КР и как
они соотносятся с нормами.

### 9.6. Детализация методики экспертных оценок

Метод экспертных оценок — основной инструмент оценки в КР, поэтому его
нужно описать строго. Ниже — предлагаемая схема.

**Шаг 1. Формирование группы.** Минимум 5 человек (рекомендуется 5–7).
Критерий включения: способность оценить интерфейс (студенты IT/робото-
технических направлений). Критерий исключения: участие в разработке
оцениваемого интерфейса (чтобы избежать предвзятости).

**Шаг 2. Выбор критериев.** Критерии берутся из раздела
«Эффективность»: скорость считывания, точность, нагрузка на внимание,
наглядность, удобство при аномалии.

**Шаг 3. Шкала оценивания.** Единая шкала для всех критериев
(например, 5-балльная), с описанием, что означает каждый балл.

**Шаг 4. Предъявление.** Экспертам показываются все варианты ИМ в
одинаковых условиях, в одном порядке (или со сменой порядка для
компенсации эффекта последовательности).

**Шаг 5. Обработка.** Расчёт средних оценок по каждому критерию и
каждому варианту, а также показателя согласованности экспертов.

**Шаг 6. Интерпретация.** Если согласованность высокая — вывод
устойчив. Если низкая — обсуждаются причины расхождений.

### 9.7. Что даёт каждый метод

| Метод | Что подтверждает | Что не подтверждает |
|---|---|---|
| Нормы эргономики | соответствие известным требованиям | превосходство варианта |
| Экспертные оценки | субъективное предпочтение | объективную скорость |
| Замер на записи | реакцию на реальные данные | реакцию живого человека |
| Комбинация | комплексную обоснованность | полноценный эксперимент |

### 9.8. План оценки (черновик)

```mermaid
graph TB
    S1["Формирование<br/>группы (5-7)"] --> S2["Выбор<br/>критериев"]
    S2 --> S3["Шкала<br/>оценивания"]
    S3 --> S4["Предъявление<br/>вариантов"]
    S4 --> S5["Сбор<br/>оценок"]
    S5 --> S6["Обработка<br/>(средние +<br/>согласованность)"]
    S6 --> S7["Интерпретация<br/>и вывод"]
```

### 9.9. Пример расчёта (иллюстративный)

Пусть три варианта ИМ оценены пятью экспертами по критерию «скорость
считывания» (5 — отлично, 1 — плохо). Средние оценки:

| Вариант | Средняя оценка | Вывод |
|---|---|---|
| Числовая панель | 3.2 | ниже среднего |
| Графическая | 3.8 | средний |
| Комбинированная | 4.4 | лучший |

Такой результат (при высокой согласованности) даёт основание
рекомендовать комбинированный вариант. Числа здесь иллюстративны и
будут заменены на реальные после оценки.
...
|---|---|---|
| «состояния робота» | слишком широко | сузить до конкретных параметров |
| «разработка и оценка» | две задачи в одной | оставить оценку/выбор |
| нет критериев эффективности | недоказуемо | добавить измеримость |
...
Комбинация В1 и В3: **«Эргономическая оценка информационной модели
критичных параметров состояния шагающего робота: сравнение форм
представления»**. Это соединяет конкретный объект (критичные параметры)
с методом (сравнение форм) и даёт измеримость.

```mermaid
graph TB
    B1["В1:<br/>критичные<br/>параметры"] --> R["Рекомендуемая<br/>комбинация"]
    B3["В3:<br/>формы<br/>представления"] --> R
    R --> O["Объект: ИМ<br/>критичных параметров"]
    R --> M["Метод: сравнение<br/>форм отображения"]
    R --> C["Критерии: скорость,<br/>точность, нагрузка"]
```
...
> **Эргономическая оценка информационной модели критичных параметров
> состояния шагающего робота для оператора: сравнение форм представления
> телеметрии**
...
| Вопрос | Ответ |
|---|---|
| Объект | Информационная модель критичных параметров состояния робота |
| Где анализ | Сравнение форм представления телеметрии |
| Характеристики | Время считывания, точность, нагрузка на внимание |
...
graph TB
    ...
    subgraph P2["Часть 2 — практическая"]
        ...
        O["Эргономическая<br/>оценка вариантов"]
        R["Рекомендация"]
    end
    P1 --> P2
```
...
несколько раз советует «посоветоваться с Антоном», «поговорить с
Антоном вечером». Это указывает на то, что:

- тема КР должна быть согласована с научруком;
- научрук знает техническую часть диплома и поможет сформулировать;
- возможно, у научрука есть видение, как связать КР и диплом.

### 13.2. Что нужно согласовать с Каневым
...
| Доступные данные телеметрии | источник для оценки |
| Требования к интерфейсу оператора | согласовать с реальным дипломом |
| Метод оценки | научрук оценит корректность |

### 13.3. Схема согласования

```mermaid
graph LR
    G["Горячкин:<br/>сузить тему"] --> S["Студент"]
    S --> K["Канев А.И.<br/>научрук"]
    K --> T["Согласованная<br/>формулировка"]
    T --> W["Работа<br/>над КР"]
...
| 3 | Согласовать с Каневым А.И. | одобренная тема | неделя |
| 4 | Определить критичные параметры | перечень | неделя |
| 5 | Продумать метод оценки | методика | 2 недели |
| 6 | Подготовить постановку (3 вопроса) | текст | 2 недели |

### 14.2. Среднесрочные шаги

| № | Действие | Результат |
|---|---|---|
| 7 | Собрать данные телеметрии | логи для оценки |
| 8 | Сделать варианты форм | прототипы/описания |
| 9 | Провести оценку | результаты |
...
graph LR
    ...
    B --> C["Согласование<br/>с Каневым"]
    C --> D["Постановка<br/>задачи"]
    D --> E["Метод<br/>оценки"]
    E --> F["Оценка<br/>вариантов"]
    F --> G["РПЗ"]
```
...
|---|---|---|---|
| Тема всё ещё покажется широкой | средняя | высокое | согласовать с Каневым до написания |
| Метод оценки отвергнут | средняя | высокое | подготовить обоснование по нормам |
| Нет реального интерфейса оператора | средняя | среднее | использовать RViz как прототип |
| Экспертная группа не соберётся | высокая | среднее | минимум 5 человек, заранее |
| Данные телеметрии неполные | низкая | среднее | зафиксировать логи заранее |
| Расхождение КР и диплома | средняя | высокое | единая постановка с научруком |
...
graph TB
    ...
    R --> R2["Метод<br/>оценки"]
    R --> R3["Данные"]
    R --> R4["Согласование"]
    R1 --> M1["Согласовать<br/>с Каневым"]
    R2 --> M2["Опора на<br/>нормы"]
...
1. Какую из трёх суженных формулировок предпочтёт Канев А.И.?
2. Достаточно ли метода экспертных оценок без психофизического
   эксперимента?
3. Считать ли RViz «экраном оператора» для целей КР?
4. Нужны ли отдельные прототипы форм представления или достаточно
   описания?
5. Какой минимальный объём экспертной группы примет Горячкин?
...
3. Эффективность — **измеримые критерии** восприятия (время, точность,
   нагрузка).
4. Метод — **аналитическая оценка + экспертные оценки + иллюстрация на
   реальных данных**.
5. Согласование — **обязательно с научным руководителем Каневым А.И.**

Следующий шаг — выбрать суженную формулировку и согласовать её с
