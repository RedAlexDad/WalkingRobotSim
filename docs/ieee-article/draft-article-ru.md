...
### 4.6. Телеметрия и воспроизводимость

Для количественного сравнения разработана единая система телеметрии,
записывающая 125 величин на каждом шаге физики в формате CSV: положение
и ориентацию корпуса (кватернион и углы Эйлера), линейные и угловые
скорости в мировой и связанной системах координат, ускорения и
направление гравитации в связанной системе, командные скорости и режим
работы контроллера, мировые положения и контакты четырёх стоп, углы и
скорости двенадцати суставов, оценку моментов, мгновенные мощности и
накопленную энергию, ошибки слежения по суставам, а также флаги падения,
насыщения hip и наличия NaN. Единый формат используется как для
модельного, так и для обученного контроллера, что обеспечивает
корректность сравнения. Момент сустава оценивается как
`τ_i = kp·(cmd_i − q_i) − kd·q̇_i`, где `cmd_i` — целевой угол,
`q_i` — фактический, `q̇_i` — скорость сустава; коэффициенты `kp` и `kd`
задаются отдельно для каждого контроллера. Энергозатраты оцениваются как
интеграл суммы мощностей по суставам, а Cost of Transport — как
`E / (m·g·d)`.

Для устранения влияния производительности на результаты контроллер
модельной походки переведён на шаг по сим-времени: среда публикует
текущее сим-время в отдельном топике, а контроллер выполняет очередной
такт только при приросте сим-времени на заданный период. Это делает
поведение детерминированным и не зависящим от частоты кадров симуляции и
загрузки процессора. Дополнительно запись телеметрии вынесена в фоновый
поток, чтобы не блокировать цикл симуляции, а перевод данных с GPU на CPU
выполняется одним пакетом за шаг. Все эксперименты выполнены на одной
версии ассета, с фиксированными параметрами физики и одинаковым
протоколом запуска, что обеспечивает воспроизводимость.
...
**Практические рекомендации.** Для задач на ровной поверхности с
фиксированной скоростью, где важны прозрачность, воспроизводимость и
отсутствие зависимости от GPU, модельный контроллер достаточен и служит
удобным эталоном. Для нерегулярного рельефа, переменных скоростей и
устойчивости к возмущениям предпочтительна обученная политика. Наиболее
перспективна гибридная схема: обученная политика как основной режим и
детерминированный модельный контроллер как резервный режим (fallback) с
автоматическим переключением по крену, току или потере контакта. Такая
схема сочетает гибкость обучения с гарантированной предсказуемостью
модельного подхода.

**Направления будущей работы.** Первое — реализовать балансир типа
capture-point и turning-контроллер, планирующий постановку стоп с учётом
боковой скорости корпуса и ошибки курса, чтобы устранить остаточный
крен и дрейф модельного контроллера. Второе — выровнять условия
сравнения, запустив оба контроллера на одном ассете с одинаковыми
PD-коэффициентами. Третье — провести серию экспериментов с внешними
возмущениями (боковой толчок, скользкая поверхность) и измерить время
восстановления обоих режимов. Четвёртое — перенести результат на
реального робота (sim-to-real) и проверить, сохраняются ли выявленные
закономерности при наличии шума сенсоров и задержек управления.
