...
**Статья:** `draft-article-en.md` (тело **3998** слов, abstract 250) —
полная, соответствует требованиям. `draft-article-ru.md` (2564 слова) —
перевод, требует синхронизации новых разделов (§5.4–5.6, §6.1–6.2, §1.1).

**Структура (EN):** Introduction (+вклад, постановка, RQ1–4), Related
Work, Architecture, Method (+5 дефектов, телеметрия), Experiments
(+таблицы, рабочая область, метрики, таксономия отказов, негативные
результаты, воспроизводимость), Discussion (+линейный анализ, P1–P4),
Conclusion, 15 ссылок.

| # | Пункт | Статус | Где |
|---|---|---|---|
| 1.2 | CoT | ✅ RL 2.84, IK 2.83 ± 0.41 | Таблица 1 |
| 1.4 | Ссылки ≥8 | ✅ 15 ссылок | `refs/references.md` |
| 2.3 | Рисунки | ✅ IK/RL траектории, диагностика крена | `reports/isaam/figures/` |
| 2.1 | Серия скоростей | ✅ рабочая область (IK ~0.3, RL 0.1–0.4) | §5.3 |
| 1.3 | Возмущения | 🟡 механизм `/robot1/push` готов, серия ⏳ | `run_sim_telemetry.py` |
| 1.1 | Ассет/PD | ✅ оформлено как ограничение | §6 Limitations |
| 2.4, 3 | Limitations + «крючок» | ✅ + научные положения P1–P4 | §6, §6.2 |
| 7.4 | Таксономия отказов + негативные | ✅ | §5.5 |
| 7.5 | Метрики (jerk, энергия по суставам) | ✅ | §5.4 |
| 7.2 | Анализ hip-петли | ✅ | §6.1 |
| 7.8 | Научные положения | ✅ P1–P4 | §6.2 |
| 7.7 | Воспроизводимость | ✅ | §5.6 |
| 7.1 | Свип по fps | 🟡 троттлинг готов, эксперимент ⏳ | `GO2_TARGET_FPS` |
| 7.6 | Третий базлайн | ⏳ новая походка (Rust) | — |
| 4 | Формальности (вёрстка, авторы, вычитка) | ⏳ | — |
...
- Рабочая область: IK устойчив только ~0.3 м/с; RL отслеживает 0.1–0.4 м/с.
- 5 дефектов интеграции + архитектурный крен через hip (roll 50→8°).
- P1: модельный контроллер привязан к реальному времени (wall-clock), RL — нет.
