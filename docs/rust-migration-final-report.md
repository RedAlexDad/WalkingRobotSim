...
- Итог: `cargo test --workspace` = **59 unit + 21 cross-val (реальный C++ харнесс) + 4 crawl + 4 odometry**,
...
- **FIX stall-логики (SLAM «белый круг по центру»)**: старый критерий считал тело
  «стоящим» только по угловой скорости IMU (`|angular| < 0.05`). При прямолинейном
  движении тело не вращается → ноги двигаются + «тело не вращается» → ложное
  застревание через `stall_window` (20) отсчётов → odom заморожен → SLAM рисовал
  карту только вокруг точки старта. Исправлено в C++ и Rust: stall срабатывает
  только если **команды движения нет** (`linear_velocity_x/y ≈ 0`) и IMU не вращается.
  Добавлен регрессионный тест `test_no_stall_when_command_given`.

### 12.5. Живая диагностика в симуляции (2026-08-20): зомби-процессы
- **Симптом:** «робот бежит сразу при спавне без команды, спотыкается; пробел в
  teleop не останавливает; SLAM рисует белую карту по центру».
- **Причина 1 (бег):** зомби `ros2 topic pub -r 10 /robot1/robot_velocity x:0.1`
  (PID 2664), оставленный `scripts/verify_rust_controller.sh` (шаг 7 запускал
  `docker exec ... &` и убивал только docker-обёртку, а не `ros2 topic pub` внутри
  контейнера). Робот получал 0.1 м/с постоянно → бежал при спавне, спотыкался при
  падении с z=0.8, пробел не помогал (зомби перебивал нулевую команду).
  **Фикс:** pub+kill внутри одной bash-сессии контейнера + `pkill` (коммит `73a8ef8`).
- **Причина 2 (белый круг):** см. FIX stall-логики в §12.4; плюс у запущенного
  odometry node отсутствовала imu-подписка (процесс держал старый бинарник), из-за
  чего `imu_angular_velocity` всегда 0 и stall-выход (требует > 0.1) не срабатывал.
  После перезапуска с актуальным бинарником + фиксом логики — odom интегрируется.
