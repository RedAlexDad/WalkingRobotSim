...
if GPU_AVAILABLE:
    ...
    def error_counting_kernel_cuda(
        resolution,
        width,
        height,
        sensor_noise_factor,
    ...
    def average_map_kernel_cuda(width, height, max_variance, initial_variance):
    ...
    def dilation_filter_kernel_cuda(width, height, dilation_size):
    ...
    def normal_filter_kernel_cuda(width, height, resolution):
    ...
    def polygon_mask_kernel_cuda(width, height, resolution):
            ...
            operation=string.Template("""
                ...
                if (p.x < bmin.x || p.x > bmax.x || p.y < bmin.y || p.y > bmax.y){
                    ...
                    for (int j = 0; j < polygon_n[0]; j++) {
                        Point p1, p2;
                        int i1 = get_idx(polygon[j, 0], polygon[j, 1], center_x[0], center_y[0]);
                        p1.x = get_idx_x(i1);
                        p1.y = get_idx_y(i1);
                        int j2 = (j + 1) % polygon_n[0];
                        int i2 = get_idx(polygon[j2, 0], polygon[j2, 1], center_x[0], center_y[0]);
                        p2.x = get_idx_x(i2);
                        p2.y = get_idx_y(i2);
    ...
    if __name__ == "__main__":
        for i in range(10):
            ...
            print(polygon_bbox)
            polygon_mask = polygon_mask_kernel_cuda(100, 100, 0.1)
            import time
...
def _make_add_points_cpu(
    ...
    def add_points_cpu(center_x, center_y, R, t, norm_map, p, map_, newmap, size=None):
        n_points = p.shape[0]
        Rf = R.ravel()
        tf = t.ravel()
        ...
        map_h = map_[0, rows, cols]
        map_v = map_[1, rows, cols]

        is_outlier = np.abs(map_h - z) > (map_v * mahalanobis_thresh)
        ...
        if np.any(inlier_mask):
            ...
            current_h = map_[0, ri, ci]
            current_v = map_[1, ri, ci]

            new_h = (current_h * vi + zi * current_v) / (current_v + vi)
            new_v = (current_v * vi) / (current_v + vi)

            np.add.at(newmap, (0, ri, ci), new_h)
            np.add.at(newmap, (1, ri, ci), new_v)
            np.add.at(newmap, (2, ri, ci), 1.0)
            map_[2, ri, ci] = 1
            map_[4, ri, ci] = 0.0
            map_[5, ri, ci] = new_h
            map_[6, ri, ci] = 0.0
        ...
        p[:, 0] = rows * width + cols
        p[:, 1] = valid.astype(float)
        p[:, 2] = inside.astype(float)

    return add_points_cpu


def _make_error_counting_cpu(
    resolution,
    width,
    height,
    sensor_noise_factor,
...
def _make_average_map_cpu(width, height, max_variance, initial_variance):
    def average_map_cpu(newmap, map_, size=None):
        cell_n = width * height
        new_h = newmap[0].ravel()
        new_v = newmap[1].ravel()
        new_cnt = newmap[2].ravel()
        map_h = map_[0].ravel()
        map_v = map_[1].ravel()
        ...
        map_h[update_mask] = new_h[update_mask] / new_cnt[update_mask]
        map_v[update_mask] = new_v[update_mask] / new_cnt[update_mask]
        map_valid[update_mask] = 1

        map_h[reset_mask] = 0
        map_v[reset_mask] = initial_variance
        map_valid[reset_mask] = 0

        no_data = ~cnt_mask & (map_valid < 0.5)
        map_h[no_data] = 0
        map_v[no_data] = initial_variance
        map_valid[no_data] = 0

    return average_map_cpu
...
def _make_normal_filter_cpu(width, height, resolution):
    def normal_filter_cpu(map_, mask, newmap, size=None):
        ...
        newmap[0] = np.where(valid, nx / norm, 0.0)
        newmap[1] = np.where(valid, ny / norm, 0.0)
        newmap[2] = np.where(valid, nz / norm, 0.0)

    return normal_filter_cpu
...
def _make_polygon_mask_cpu(width, height, resolution):
    def polygon_mask_cpu(polygon, center_x, center_y, polygon_n, polygon_bbox, mask, size=None):
        ...
        bbox_min_y = (polygon_bbox[1] - cy) / resolution + 0.5 * height
        bbox_max_x = (polygon_bbox[2] - cx) / resolution + 0.5 * width
        bbox_max_y = (polygon_bbox[3] - cy) / resolution + 0.5 * height

        bmin_x = int(max(0, bbox_min_x))
        bmin_y = int(max(0, bbox_min_y))
        bmax_x = int(min(width - 1, bbox_max_x))
        bmax_y = int(min(height - 1, bbox_max_y))

        ys, xs = np.mgrid[bmin_y : bmax_y + 1, bmin_x : bmax_x + 1]
        points = np.column_stack([xs.ravel(), ys.ravel()])

        path = Path(vertices)
        in_polygon = path.contains_points(points).reshape(xs.shape)

        mask[:] = 0.0
        mask[bmin_y : bmax_y + 1, bmin_x : bmax_x + 1] = in_polygon.astype(float)

    return polygon_mask_cpu
...
def error_counting_kernel(
    ...
    if GPU_AVAILABLE:
        return error_counting_kernel_cuda(
            resolution,
            width,
            height,
            sensor_noise_factor,
    ...
    return _make_error_counting_cpu(
        resolution,
        width,
        height,
        sensor_noise_factor,
...
def average_map_kernel(width, height, max_variance, initial_variance):
    if GPU_AVAILABLE:
        return average_map_kernel_cuda(width, height, max_variance, initial_variance)
    return _make_average_map_cpu(width, height, max_variance, initial_variance)


def dilation_filter_kernel(width, height, dilation_size):
    if GPU_AVAILABLE:
        return dilation_filter_kernel_cuda(width, height, dilation_size)
    return _make_dilation_filter_cpu(width, height, dilation_size)


def normal_filter_kernel(width, height, resolution):
    if GPU_AVAILABLE:
        return normal_filter_kernel_cuda(width, height, resolution)
    return _make_normal_filter_cpu(width, height, resolution)


def polygon_mask_kernel(width, height, resolution):
    if GPU_AVAILABLE:
        return polygon_mask_kernel_cuda(width, height, resolution)
    return _make_polygon_mask_cpu(width, height, resolution)
