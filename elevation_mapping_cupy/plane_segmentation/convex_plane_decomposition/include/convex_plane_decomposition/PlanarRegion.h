...
struct PlanarRegion {
    ...
    /// 3D Transformation from terrain to world. v_world = T * v_plane. Use .linear() for the rotation and
 ...
 *
 * For example, if the normal and position are defined in world. We return a transform T, such that v_world = T *
 * v_plane. The normal will be taken as the z-direction of the local frame. The x and y direction are arbitrary.
 */
 ...
 */
Eigen::Vector3d positionInWorldFrameFromPosition2dInPlane(const CgalPoint2d& planeXY,
