...
struct SlidingWindowPlaneExtractorParameters {
    ...
    /// [-] Maximum allowed angle between the surface normal and the world-z direction for a patch (converted to
    /// dotproduct bound)
    double plane_inclination_threshold = std::cos(30.0 * M_PI / 180.0);

    /// [-] Maximum allowed angle between the surface normal and the world-z direction for a cell (converted to
    /// dotproduct bound)
    ...
    /// [deg] Allowed normal vector deviation for a point w.r.t. the plane normal. If any point violates this, RANSAC is
    /// triggered
