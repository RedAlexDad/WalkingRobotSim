...
std::array<CgalPoint2d, 4> get2ndNeighbours(const CgalPolygon2d& polygon, int i) {
    ...
    return {polygon.vertex(cwNeighbour2), polygon.vertex(cwNeighbour1), polygon.vertex(ccwNeighbour1),
            polygon.vertex(ccwNeighbour2)};
}
 ...
 * Returns {false, minSquareDistance} if none of the segments intersects. minSquareDistance is the minimum square
 * distance between the point and the parent shape
 */
template <typename T>
std::pair<bool, double> doEdgesIntersectAndSquareDistance(const std::array<CgalPoint2d, 2>& neighbours,
                                                          const CgalPoint2d& point, const T& parentShape);

template <>
std::pair<bool, double> doEdgesIntersectAndSquareDistance(const std::array<CgalPoint2d, 2>& neighbours,
                                                          const CgalPoint2d& point, const CgalPolygon2d& parentShape) {
...
template <>
std::pair<bool, double> doEdgesIntersectAndSquareDistance(const std::array<CgalPoint2d, 2>& neighbours,
                                                          const CgalPoint2d& point,
                                                          const CgalPolygonWithHoles2d& parentShape) {
    const auto intersectAndDistance =
        doEdgesIntersectAndSquareDistance(neighbours, point, parentShape.outer_boundary());
...
template <typename T>
bool pointCanBeMoved(const CgalPolygon2d& growthShape, int i, const CgalPoint2d& candidatePoint,
                     CgalCircle2d& freeSphere, const T& parentShape) {
    if (remainsConvexWhenMovingPoint(growthShape, i, candidatePoint)) {
        ...
        if (pointAndNeighboursAreWithinFreeSphere(neighbours, candidatePoint, freeSphere)) {
            ...
            const auto intersectAndDistance =
                doEdgesIntersectAndSquareDistance(neighbours, candidatePoint, parentShape);
...
template <typename T>
CgalPolygon2d growConvexPolygonInsideShape_impl(const T& parentShape, CgalPoint2d center, int numberOfVertices,
                                                double growthFactor) {
    ...
    if (radius == 0.0) {
        std::cerr << "[growConvexPolygonInsideShape] Zero initial radius. Provide a point with a non-zero offset to "
                     "the boundary.\n";
        return growthShape;
    }
...
CgalPolygon2d growConvexPolygonInsideShape(const CgalPolygonWithHoles2d& parentShape, CgalPoint2d center,
                                           int numberOfVertices, double growthFactor) {
    return growConvexPolygonInsideShape_impl(parentShape, center, numberOfVertices, growthFactor);
}
