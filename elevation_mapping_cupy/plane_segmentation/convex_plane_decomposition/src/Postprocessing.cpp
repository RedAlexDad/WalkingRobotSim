...
void Postprocessing::dilationInNonplanarRegions(Eigen::MatrixXf& elevationData,
    ...
    if (parameters_.nonplanar_horizontal_offset > 0) {
        ...
        elevationData =
            planarityMask.array() * elevationData.array() + (1.0 - planarityMask.array()) * elevationDilated.array();
    }
}

void Postprocessing::addHeightOffset(Eigen::MatrixXf& elevationData, const Eigen::MatrixXf& planarityMask) const {
    // lift elevation layer. For untraversable offset we first add the offset everywhere and substract it again in
    // traversable regions.
...
void Postprocessing::addSmoothLayer(grid_map::GridMap& gridMap, const Eigen::MatrixXf& elevationData,
    ...
    const int dilationType = cv::MORPH_ELLIPSE;  // ellipse inscribed in the square of size dilationSize
    const auto dilationKernel_ = cv::getStructuringElement(dilationType, cv::Size(dilationSize, dilationSize));
    cv::morphologyEx(elevationWithNaNCv, elevationWithNaNCv, cv::MORPH_CLOSE, dilationKernel_, cv::Point(-1, -1), 1,
                     cv::BORDER_REPLICATE);
    ...
    grid_map::processing::applyKernelFunction(gridMap, "elevationWithNaNClosed", "elevationWithNaNClosedDilated",
                                              dilationSize,
                                              [&](const Eigen::Ref<const grid_map::GridMap::Matrix>& data) -> float {
                                                  return (data - offsets).maxCoeffOfFinites();
                                              });
    ...
    auto smoothingFilter = [kernel, kernelGauss](const cv::Mat& imageIn) {
        cv::Mat imageOut;
        cv::boxFilter(imageIn, imageOut, -1, {kernel, kernel}, cv::Point(-1, -1), true,
