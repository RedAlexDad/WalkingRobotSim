...
const CgalPolygonWithHoles2d* findInsetContainingThePoint(const CgalPoint2d& point,
                                                          const std::vector<CgalPolygonWithHoles2d>& insets) {
...
CgalPoint2d projectToPlanarRegion(const CgalPoint2d& queryProjectedToPlane, const PlanarRegion& planarRegion) {
    ...
    // Note: we know that all insets are non-overlapping, and are not nested (no shape is contained in the hole of
    // another shape)
    const auto* const insetPtrContainingPoint =
        findInsetContainingThePoint(queryProjectedToPlane, planarRegion.boundaryWithInset.insets);
...
std::vector<RegionSortingInfo> sortWithBoundingBoxes(
    const Eigen::Vector3d& positionInWorld,
...
PlanarTerrainProjection getBestPlanarRegionAtPositionInWorld(
    const Eigen::Vector3d& positionInWorld, const std::vector<PlanarRegion>& planarRegions,
    ...
    for (const auto& regionInfo : sortedRegions) {
        ...
        const auto projectedPointInTerrainFrame =
            projectToPlanarRegion(regionInfo.positionInTerrainFrame, *regionInfo.regionPtr);

        // Express projected point in World frame
        const auto projectionInWorldFrame = positionInWorldFrameFromPosition2dInPlane(
            projectedPointInTerrainFrame, regionInfo.regionPtr->transformPlaneToWorld);

        const auto cost =
            distanceCost(positionInWorld, projectionInWorldFrame) + penaltyFunction(projectionInWorldFrame);
