...
std::vector<PlanarRegion> ContourExtraction::extractPlanarRegions(const SegmentedPlanesMap& segmentedPlanesMap) {
    ...
    planarRegions.reserve(upSampledMap.highestLabel +
                          1);  // Can be more or less in the end if regions are split or removed.
...
CgalPoint2d pixelToWorldFrame(const CgalPoint2d& pixelspaceCgalPoint2d, double resolution,
    ...
    return {mapOffset.x() - resolution * pixelspaceCgalPoint2d.y(),
            mapOffset.y() - resolution * pixelspaceCgalPoint2d.x()};
}
