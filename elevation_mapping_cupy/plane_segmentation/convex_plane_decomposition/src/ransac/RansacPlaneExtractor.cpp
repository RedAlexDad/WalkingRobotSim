...
void RansacPlaneExtractor::setParameters(const RansacPlaneExtractorParameters& parameters) {
    ...
    cgalRansacParameters_.epsilon =
        parameters.epsilon / 3.0;  // CGAL ransac puts the inlier tolerance at 3 times epsilon
...
std::pair<Eigen::Vector3d, Eigen::Vector3d> RansacPlaneExtractor::getPlaneParameters(Shape* shapePtr) {
    ...
    Eigen::Vector3d normalVector(planePtr->plane_normal().x(), planePtr->plane_normal().y(),
                                 planePtr->plane_normal().z());
