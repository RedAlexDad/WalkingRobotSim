...
SlidingWindowPlaneExtractor::SlidingWindowPlaneExtractor(
...
std::pair<Eigen::Vector3d, double> SlidingWindowPlaneExtractor::computeNormalAndErrorForWindow(
    const Eigen::MatrixXf& windowData) const {
    ...
    for (int kernel_col = 0; kernel_col < parameters_.kernel_size; ++kernel_col) {
        for (int kernel_row = 0; kernel_row < parameters_.kernel_size; ++kernel_row) {
            ...
            Eigen::Vector3d point{-kernel_row * segmentedPlanesMap_.resolution,
                                  -kernel_col * segmentedPlanesMap_.resolution, height};
            nPoints++;
            sum += point;
...
void SlidingWindowPlaneExtractor::runSlidingWindowDetector() {
    grid_map::SlidingWindowIterator window_iterator(
        *map_, elevationLayer_, grid_map::SlidingWindowIterator::EdgeHandling::EMPTY, parameters_.kernel_size);
    ...
    if (parameters_.planarity_opening_filter > 0) {
        ...
        const auto kernel_ =
            cv::getStructuringElement(openingKernelType, cv::Size(openingKernelSize, openingKernelSize));
        cv::morphologyEx(binaryImagePatch_, binaryImagePatch_, cv::MORPH_OPEN, kernel_, cv::Point(-1, -1), 1,
                         cv::BORDER_REPLICATE);
    }
}
...
void SlidingWindowPlaneExtractor::runSegmentation() {
    int numberOfLabel =
        cv::connectedComponents(binaryImagePatch_, segmentedPlanesMap_.labeledImage, parameters_.connectivity, CV_32S);
...
void SlidingWindowPlaneExtractor::computePlaneParametersForLabel(
    int label, std::vector<ransac_plane_extractor::PointWithNormal>& pointsWithNormal) {
            ...
            if (segmentedPlanesMap_.labeledImage.at<int>(row, col) == label) {
                ...
                if (std::isfinite(height)) {
                    const Eigen::Vector3d point3d{
                        segmentedPlanesMap_.mapOrigin.x() - row * segmentedPlanesMap_.resolution,
                    ...
                    pointsWithNormal.emplace_back(
                        ...
                        ransac_plane_extractor::Vector3D(localSurfaceNormal.x(), localSurfaceNormal.y(),
    ...
    if (parameters_.include_ransac_refinement) {                                // with RANSAC
        if (isGloballyPlanar(normalVector, supportVector, pointsWithNormal)) {  // Already planar enough
            if (isWithinInclinationLimit(normalVector)) {
                segmentedPlanesMap_.labelPlaneParameters.emplace_back(label,
                                                                      NormalAndPosition{supportVector, normalVector});
            } else {
...
void SlidingWindowPlaneExtractor::refineLabelWithRansac(
    ...
    for (const auto& plane : planes) {
        ...
        if (isWithinInclinationLimit(normalVector)) {
            ...
            segmentedPlanesMap_.labelPlaneParameters.emplace_back(newLabel,
                                                                  NormalAndPosition{supportVector, normalVector});
...
void SlidingWindowPlaneExtractor::addSurfaceNormalToMap(grid_map::GridMap& map, const std::string& layerPrefix) const {
    ...
    grid_map::SlidingWindowIterator window_iterator(
        map, layerPrefix + "_x", grid_map::SlidingWindowIterator::EdgeHandling::EMPTY, parameters_.kernel_size);
...
bool SlidingWindowPlaneExtractor::isGloballyPlanar(
    const Eigen::Vector3d& normalVectorPlane, const Eigen::Vector3d& supportVectorPlane,
    ...
    const double dotProductThreshold =
        std::cos(parameters_.global_plane_fit_angle_error_threshold_degrees * M_PI / 180.0);

    for (const auto& pointWithNormal : pointsWithNormal) {
        const double normalDotPoint = normalVectorPlane.x() * pointWithNormal.first.x() +
                                      normalVectorPlane.y() * pointWithNormal.first.y() +
        ...
        if (distanceError > parameters_.global_plane_fit_distance_error_threshold ||
            dotProductNormals < dotProductThreshold) {
            return false;
        }
    }
