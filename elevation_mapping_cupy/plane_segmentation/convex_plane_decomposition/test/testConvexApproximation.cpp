 ...
 */
PlanarTerrainProjection getBestPlanarRegionAtPositionInWorldNaive(
    ...
    std::for_each(planarRegions.begin(), planarRegions.end(), [&](const PlanarRegion& planarRegion) {
        ...
        projection.positionInTerrainFrame =
            projectToPlanarRegion({positionInTerrainFrame.x(), positionInTerrainFrame.y()}, planarRegion);
        projection.positionInWorld = positionInWorldFrameFromPosition2dInPlane(projection.positionInTerrainFrame,
                                                                               planarRegion.transformPlaneToWorld);
        projection.cost =
            (positionInWorld - projection.positionInWorld).squaredNorm() + penaltyFunction(projection.positionInWorld);
        individualProjections.push_back(projection);
    });

    // Find the minimum cost projection
    return *std::min_element(
        individualProjections.begin(), individualProjections.end(),
        ...
        for (double y = -submapSize.y() / 2.0; y < submapSize.y() / 2.0; y += submapSize.y() / 4.0) {
            for (double z = -heightScale; z < heightScale; z += heightScale / 2.0) {
                ...
                auto penaltyFunction = [](const Eigen::Vector3d& projectedPoint) {
                    return 0.1 * std::abs(projectedPoint.z());
                };

                // Run projection and naive projection
                const auto projection =
                    getBestPlanarRegionAtPositionInWorld(query, planarTerrain.planarRegions, penaltyFunction);
                const auto projectionCheck =
                    getBestPlanarRegionAtPositionInWorldNaive(query, planarTerrain.planarRegions, penaltyFunction);
                ...
                for (int numberOfVertices = 3; numberOfVertices < 7; ++numberOfVertices) {
                    for (double growthFactor = 1.01; growthFactor < 1.3; growthFactor += 0.1) {
                        ...
                        ASSERT_TRUE(std::all_of(
                            convexRegion.vertices_begin(), convexRegion.vertices_end(), [&](const CgalPoint2d& p) {
                                return isInside(p, projection.regionPtr->boundaryWithInset.boundary);
                            }));
                    }
                }
            }
