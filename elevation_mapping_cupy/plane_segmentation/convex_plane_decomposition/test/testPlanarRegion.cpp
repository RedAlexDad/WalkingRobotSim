...
TEST(TestPlanarRegion, projectPositionInWorldOntoPlaneAlongGravity) {
    const NormalAndPosition normalAndPosition = {Eigen::Vector3d(0.1, 0.2, 0.3),
                                                 Eigen::Vector3d(0.4, 0.5, 0.6).normalized()};
    ...
    const CgalPoint2d projectedPositionInPlane =
        projectToPlaneAlongGravity({queryPosition.x(), queryPosition.y()}, transformPlaneToWorld);
