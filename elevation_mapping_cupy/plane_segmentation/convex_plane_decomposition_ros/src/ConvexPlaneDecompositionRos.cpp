...
ConvexPlaneExtractionROS::ConvexPlaneExtractionROS(ros::NodeHandle& nodeHandle) : tfBuffer_(), tfListener_(tfBuffer_) {
    ...
    if (parametersLoaded) {
        elevationMapSubscriber_ =
            nodeHandle.subscribe(elevationMapTopic_, 1, &ConvexPlaneExtractionROS::callback, this);
...
ConvexPlaneExtractionROS::~ConvexPlaneExtractionROS() {
    if (callbackTimer_.getNumTimedIntervals() > 0 && planeDecompositionPipeline_ != nullptr) {
        ...
        infoStream << "\n########################################################################\n";
        infoStream << "The benchmarking is computed over " << callbackTimer_.getNumTimedIntervals()
                   << " iterations. \n";
...
void ConvexPlaneExtractionROS::callback(const grid_map_msgs::GridMap& message) {
    ...
    if (targetFrameId_ != messageMap.getFrameId()) {
        ...
        if (tfBuffer_.canTransform(targetFrameId_, messageMap.getFrameId(), timeStamp, &errorMsg)) {
            ...
            messageMap = grid_map::GridMapCvProcessing::getTransformedMap(std::move(messageMap), transform,
    ...
    grid_map::GridMap elevationMap =
        messageMap.getSubmap(submapPosition, Eigen::Array2d(subMapLength_, subMapWidth_), success);
    if (!success) {
    ...
    boundaryPublisher_.publish(convertBoundariesToRosMarkers(planarTerrain.planarRegions,
                                                             planarTerrain.gridMap.getFrameId(),
