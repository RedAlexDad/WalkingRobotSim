...
geometry_msgs::msg::PolygonStamped to3dRosPolygon(const CgalPolygon2d& polygon,
...
visualization_msgs::msg::Marker to3dRosMarker(const CgalPolygon2d& polygon,
                                              const Eigen::Isometry3d& transformPlaneToWorld,
                                              const std_msgs::msg::Header& header,
...
visualization_msgs::msg::MarkerArray to3dRosMarker(const CgalPolygonWithHoles2d& polygonWithHoles,
    ...
    polygons.markers.emplace_back(
        to3dRosMarker(polygonWithHoles.outer_boundary(), transformPlaneToWorld, header, color, id, lineWidth));
    ++id;
...
visualization_msgs::msg::MarkerArray convertBoundariesToRosMarkers(const std::vector<PlanarRegion>& planarRegions,
    ...
    for (const auto& planarRegion : planarRegions) {
        ...
        auto boundaries = to3dRosMarker(planarRegion.boundaryWithInset.boundary, planarRegion.transformPlaneToWorld,
