...
class GridMapDerivative {
  ...
  public:
    ...
    /**
     * @brief Compute local height gradient and curvature using grid map. Gradient and curvature are set to zero if the
     * index is outside of the grid map.
     * @param gridMap         The grid map
     * @param gradient        gradient vector in world frame
  ...
  private:
    /**
     * @brief Return center of kernel s.t. kernel does not reach boundaries of grid map. By default returns 0 (equals
     * central difference).
     * @param gridMap         The grid map
     ...
     */
    static Eigen::Vector2i getKernelCenter(const grid_map::GridMap& gridMap, const grid_map::Index& centerIndex,
                                           int maxKernelId);
