 ...
 * @date        20.05, 2021
 * @affiliation ETH RSL
 * @brief       Computes first and second order derivatives for a grid map. Intended to be used for online applications
 * (i.e, derivatives are not precomputed but computed at the time and location where they are needed).
 */
...
void GridMapDerivative::estimateGradient(const grid_map::GridMap& gridMap, Gradient& gradient,
...
void GridMapDerivative::estimateGradientAndCurvature(const grid_map::GridMap& gridMap, Gradient& gradient,
                                                     Curvature& curvature, const grid_map::Index& index,
...
Eigen::Vector2i GridMapDerivative::getKernelCenter(const grid_map::GridMap& gridMap, const grid_map::Index& centerIndex,
                                                   int maxKernelId) {
    ...
    for (auto dim = 0; dim < 2; ++dim) {
        ...
        centerId(dim) =
            -std::min(centerIndex(dim) - maxKernelId, minId) - std::max(centerIndex(dim) + maxKernelId - maxId, 0);
    }
    return centerId;
}
