...
void minValues(grid_map::GridMap& map, const std::string& layerIn, const std::string& layerOut) {
     ...
     *
     * We will be reading and writing to H_out during iteration. However, the aliasing does not break the correctness of
     * the algorithm.
     */
    ...
    for (auto colId = 0; colId < H_in.cols(); ++colId) {
        for (auto rowId = 0; rowId < H_in.rows(); ++rowId) {
            if (std::isnan(H_in(rowId, colId))) {
                ...
                H_out(rowId, colId) =
                    weights.dot(Eigen::Vector4f(1.0, static_cast<float>(index0.x()), static_cast<float>(index0.y()),
...
void nonlinearInterpolation(grid_map::GridMap& map, const std::string& layerIn, const std::string& layerOut,
    ...
    // Create mask.
    Eigen::Matrix<uchar, -1, -1> mask =
        H_in.unaryExpr([](float val) { return (std::isnan(val)) ? uchar(1) : uchar(0); });
    cv::Mat maskImage;
    ...
    grid_map::GridMapCvConverter::toImage<unsigned char, 1>(map, layerIn, CV_8UC1, minValue, maxValue,
                                                            elevationImageIn);
