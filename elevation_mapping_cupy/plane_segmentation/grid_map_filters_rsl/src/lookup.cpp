...
LookupResult maxValueBetweenLocations(const grid_map::Position& position1, const grid_map::Position& position2,
    ...
    // Map corner points into grid map. The line iteration doesn't account for the case where the line does not
    // intersect the map.
...
std::vector<grid_map::Position3> valuesBetweenLocations(const grid_map::Position& position1,
...
grid_map::Position projectToMapWithMargin(const grid_map::GridMap& gridMap, const grid_map::Position& position,
                                          double margin) {
