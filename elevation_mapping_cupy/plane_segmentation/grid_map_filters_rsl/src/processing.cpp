...
void dilate(grid_map::GridMap& map, const std::string& layerIn, const std::string& layerOut,
            const grid_map::Matrix& mask, int kernelSize, bool inpaint) {
        ...
        for (auto rowId = 0; rowId < H_in.rows(); ++rowId) {
            ...
            if (inpaint || !std::isnan(H_in(rowId, colId))) {
                const auto maxInKernel =
                    H_in_masked.block(cornerRowId, cornerColId, kernelSize, kernelSize).maxCoeffOfFinites();
        ...
        for (auto rowId = 0; rowId < H_in.rows(); ++rowId) {
            ...
            if (inpaint || !std::isnan(H_in(rowId, colId))) {
                const auto minInKernel =
                    H_in_masked.block(cornerRowId, cornerColId, kernelSize, kernelSize).minCoeffOfFinites();
