...
#### 12.2. Загрузите маршрут и запустите полный цикл эксперимента

На хосте выполните одной командой:

```bash
make experiment-run FILE=straight_route
```

Эта команда выполнит последовательно:
1. Загрузит waypoints из файла `straight_route`
2. Запустит логгер эксперимента (запомнит время и начальную позицию)
3. Запустит навигацию по маршруту

#### 12.3. Дождитесь завершения навигации

Наблюдайте движение робота в RViz. Когда все точки пройдены — навигация завершится автоматически.

#### 12.4. Остановите эксперимент и получите результаты

```bash
make experiment-stop
```

Результаты сохранятся в файл `/tmp/experiments/experiment_<timestamp>.txt` внутри контейнера. Просмотрите их:

```bash
make experiment-result
```

Скопируйте файл результатов на хост:

```bash
docker cp walking_robot_sim:/tmp/experiments .
```
