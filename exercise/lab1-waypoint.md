...
### Шаг 12. Простой эксперимент

Создайте файл `straight_route.yaml` с 5 точками по прямой (шаг 1 метр по X). Загрузите его и запустите навигацию. Замерьте время прохождения и пройденное расстояние:

```bash
# Терминал 1 (в make shell) — запись начальной позиции
ros2 topic echo /robot1/odom --once | grep -E "position" > start_pos.txt
date +%s > start_time.txt

# Терминал 2 (на хосте) — запуск навигации
make waypoint-start
```
...
```bash
# В make shell — запись конечной позиции
ros2 topic echo /robot1/odom --once | grep -E "position" > end_pos.txt
date +%s > end_time.txt

# Посчитать время
echo "Время: $(($(cat end_time.txt) - $(cat start_time.txt))) сек"
```

Пройденное расстояние считается по координатам из `start_pos.txt` и `end_pos.txt`.

Повторите для маршрута с большим количеством точек (10-15). Сравните результаты в таблице:

| Маршрут         | Количество точек | Время (сек) |
| --------------- | ---------------- | ----------- |
| straight_route  | 5                | ...         |
| (ваш маршрут)   | ...              | ...         |
...
4. **Метрики и анализ**:
   - Таблица времени прохождения маршрутов (из эксперимента в Шаге 12)
5. **Выводы**:
...
| Проведён эксперимент (замер времени)           | 2      |
