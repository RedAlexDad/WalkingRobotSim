...
### Сравнение маршрутов — непонятно, что замерять

**Метрика:** Используйте секундомер на телефоне или часах. Засеките время от запуска `make waypoint-start` до момента, когда робот пройдёт последнюю точку (определяется визуально в RViz).
