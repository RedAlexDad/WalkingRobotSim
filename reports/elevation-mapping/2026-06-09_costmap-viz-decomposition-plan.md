...
### Фаза 4 — Исправление: elevation_costmap_layer затирал препятствия LiDAR ✅ ВЫПОЛНЕНО

#### Проблема

После настройки RViz пользователь сообщил: «розовые динамические границы не показывают, робот не обходит препятствия, идёт прямо». Costmap overlay был виден, но occupied=0 — Nav2 не видел препятствий.

#### Диагностика

Были проверены все индивидуальные слои costmap:

| Слой                        | Топик                                            | Occupied | Описание                         |
| --------------------------- | ------------------------------------------------ | -------- | -------------------------------- |
| `voxel_layer` (local)       | `/robot1/local_costmap/voxel_layer`              | **330**  | LiDAR видит препятствия!         |
| `obstacle_layer` (global)   | `/robot1/global_costmap/obstacle_layer`          | **47**   | LiDAR видит препятствия!         |
| `elevation_costmap_layer`   | `/robot1/global_costmap/elevation_costmap_layer` | **0**    | Плоский рельеф — все клетки free |
| **combined global_costmap** | `/robot1/global_costmap/costmap`                 | **0**    | ❌ Препятствия потеряны!         |
| **combined local_costmap**  | `/robot1/local_costmap/costmap`                  | **0**    | ❌ Препятствия потеряны!         |

#### Коренная причина

`elevation_costmap_layer` использует плагин `nav2_costmap_2d::StaticLayer`, у которого по умолчанию `combination_method=0` (OVERWRITE). При размещении этого слоя **ПОСЛЕ** `voxel_layer`/`obstacle_layer` в списке plugins, он полностью перезаписывал мастер-карту своими значениями (free=0), затирая occupied-клетки, проставленные LiDAR.

Порядок ДО исправления:

```
plugins: ["static_layer", "voxel_layer", "elevation_costmap_layer", "inflation_layer"]
                                    ^^^^^^^^^^^^^^^^^^^^^^^^
                                    Затирает occupied от voxel_layer!
```

#### Исправление

Изменён порядок слоёв: `elevation_costmap_layer` перенесён **ДО** `voxel_layer`/`obstacle_layer`:

```yaml
# Local costmap
plugins: ["static_layer", "elevation_costmap_layer", "voxel_layer", "inflation_layer"]

# Global costmap
plugins: ["static_layer", "elevation_costmap_layer", "obstacle_layer", "inflation_layer"]
```

**Почему это работает:**

1. `elevation_costmap_layer` (StaticLayer, OVERWRITE) — проставляет free (0) в пустых клетках
2. `voxel_layer`/`obstacle_layer` (по умолчанию `combination_method=1` = MAX) — запускается ПОСЛЕ и применяет `max(elevation_free, obstacle_occupied)` = **100** → occupied сохраняется!

**Альтернативы, которые НЕ сработали:**

- `combination_method: 1` на elevation_costmap_layer — **не работает**, т.к. `StaticLayer` игнорирует этот параметр в некоторых версиях Nav2, всегда применяя OVERWRITE
- Изменение порогов в bridge — cost=[0.8, 1.0] корректен для плоского рельефа (1.0 = traversable), bridge с `invert=True` правильно конвертирует

#### Результат

| Слой                      | До фикса       | После фикса         |
| ------------------------- | -------------- | ------------------- |
| `voxel_layer`             | 330 occupied   | 330 occupied        |
| `elevation_costmap_layer` | 0 occupied     | 0 occupied          |
| **combined costmap**      | **0 occupied** | **330 occupied** ✅ |

В RViz появились розовые/фиолетовые границы препятствий. Nav2 начала видеть препятствия и строить пути их обхода.
...
- [x] **Фаза 4 — Исправление слоёв costmap** (выполнено 09.06.2026)
- [x] Global Costmap overlay — RViz подписан, QoS совпадает, данные отображаются
- [x] ObstacleCloud + costmap — LiDAR данные поступают, occupied отображаются (330 voxel, 47 obstacle)
- [x] Cost layer (elevation) — включён, elevation_costmap_layer в обоих costmap активен
- [x] Composite costmap — порядок слоёв исправлен: elevation ДО obstacle/voxel, occupied сохраняются
- [x] Розовые/фиолетовые границы препятствий отображаются в RViz
...
| --------------------------------------------------------- | ----------- | -------------- | ---------------------- | --------------------------------------------------------------------------------------------------- |
| **Топики Nav2 недоступны из elevation контейнера**        | Низкая      | Высокое        | ✅ **Не подтвердился** | `network_mode: host` решает                                                                         |
| **Costmap пустая (Nav2 не стартанула)**                   | Средняя     | Высокое        | ✅ **Не подтвердился** | Убедиться что Gazebo + Nav2 запущены                                                                |
| **Разные fixed frame (odom vs map)**                      | Средняя     | Среднее        | ✅ **Не подтвердился** | Fixed Frame изменён на `map`                                                                        |
| **Конфликт QoS (Reliable vs BestEffort)**                 | Низкая      | Среднее        | ✅ **Не подтвердился** | Costmap: TRANSIENT_LOCAL, RViz: TRANSIENT_LOCAL                                                     |
| **Elevation costmap layer в Nav2 не загружен**            | Средняя     | Среднее        | ✅ **Не подтвердился** | Слой active в обоих costmap                                                                         |
| **/downsampled_costmap отсутствует**                      | Низкая      | Низкое         | ✅ **Не подтвердился** | Топик публикуется                                                                                   |
| **Порядок слоёв costmap: elevation ПОСЛЕ obstacle/voxel** | 🔴 Высокая  | 🔴 Критическое | ✅ **Исправлено**      | StaticLayer (combination_method=0) затирает occupied от LiDAR. Решение: elevation ДО obstacle/voxel |
