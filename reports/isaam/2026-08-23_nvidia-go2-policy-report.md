...
| [26](#26-проблема-робот-теряет-равновесие-на-ассете-nvidia) | Робот теряет равновесие | Математический контроллер тюнен под URDF/Gazebo; hip жёсткий; IMU-компенсация не проверена | Тюнинг в следующей итерации | [ ] не решено |
...
| 26 | Робот теряет равновесие на ассете NVIDIA | (1) жёсткий hip; (2) IMU-компенсация не доходит; (3) тюнинг под другой URDF | Математический контроллер тюнен под URDF/Gazebo; на ассете NVIDIA физика другая; hip жёсткий (100); IMU-компенсация не проверена | Тюнинг в следующей итерации (мягкий hip, проверка IMU) | лог REPORT (roll растёт), сравнение poses | 🔴 |
...
Робот на ассете NVIDIA (через `isaac_rust_bridge.py`) ходит и поворачивается, но заваливается на бок:

```
[REPORT] pos=(-0.11,+0.20,+0.27) rpy=(+20.6°,+11.1°,+76.0°)  # старт, стоит
[REPORT] pos=(-0.86,-0.97,+0.27) rpy=(+18.7°,+0.5°,-58.8°)   # движется
...
- ❌ **Гипотеза C:** hip жёсткий (stiffness=100) мешает балансировке. **Частично** — в Gazebo hip был мягкий (2); на ассете не проверено.
- ❌ **Гипотеза D:** IMU-компенсация контроллера не доходит (DDS хост↔контейнер). **Не подтверждена** — топик `/robot1/imu` не проверялся при живом мосте, требует проверки.

### 26.3. Причина

Математический Rust-контроллер (TROT) настроен под физику xacro-URDF в Gazebo. Ассет NVIDIA имеет другие массы/инерции/коллизии, поэтому требуются пересмотр gains (в т.ч. hip), проверка IMU-компенсации и, возможно, калибровка под новую физику.

### 26.4. Диагностика

```
# CMD vs FACT — суставы следуют командам
CMD  FR_thigh_joint=+1.585  FR_calf_joint=-0.701 ...
FACT FR_thigh_joint=+1.047  FR_calf_joint=-0.838 ...   # близко, следует

# Робот стоит/идёт на старте, потом roll растёт до 180°
```

### 26.5. Решение

Тюнинг в следующей итерации:
1. Уменьшить скорость (vx 0.3 → меньше) и проверить гипотезу hip (мягкий stiffness=2, как в Gazebo).
2. Проверить доставку IMU до контроллера (`ros2 topic echo /robot1/imu` при живом мосте).
3. Возможно, калибровка gains/амплитуд под физику NVIDIA.

### 26.6. Исправление в скриптах/конфигах

- `src/isaac/isaac_rust_bridge.py` — gains (сейчас stiffness=100, damping=5) — кандидат на тюнинг.

### 26.7. Результат

| Метрика | До | После |
|---|---|---|
| Робот ходит/поворачивается | да | да |
| Равновесие (roll) | растёт до 180° | не решено (тюнинг) |
