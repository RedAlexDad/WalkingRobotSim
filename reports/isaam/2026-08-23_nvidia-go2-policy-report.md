...
- Кандидаты на калибровку: инверсия calf (`theta4 → -theta4`), пересмотр масштаба thigh, подбор robot_height.

Идея интеграции с IsaacLab сохранена в [A.6 «Дальнейшие шаги»](#a6-дальнейшие-шаги).

### 26.6. Исправление в скриптах/конфигах

- `src/isaac/isaac_rust_bridge.py` — gains (hip=25, upper/lower=100), расширенные лимиты [-100,100], усиленный лог ([HIP], [FEET], полная CMD в REPORT), опция `--hold-stance` для калибровки позы.
...
| Высота тела с командами контроллера | 0.08 («сидит») | 0.08 («сидит») — не решено |
| Высота тела с эталонной позой (hold-stance) | — | **0.36 (стоит!)** |
| NaN-взрыв физики | был на URDF | нет на ассете NVIDIA |

**Вывод:** поза `[0, 0.67, -1.3]` из go2_isaac_ros2 корректна для ассета (робот встаёт на Z=0.36). Осталось откалибровать IK контроллера, чтобы он выдавал эту конвенцию углов.
...
| 🟡 (1-4ч) | 5 |
| 🔴 (>4ч) | 1 |
| Ключевые выводы | Готовое решение NVIDIA (ассет Mujoco_Menagerie + RL-политика) работает «из коробки», если ассеты скачаны локально и задан asset_root. Multi-physics ассет требует выбора variant Physics. Команды политике передаются torch.Tensor на cuda. При подключении Rust-контроллера к ассету NVIDIA: собственный SingleThreadedExecutor решает смерть контекста rclpy (guard_condition невалиден); ремаппинг DOF требует правильной индексации (`targets[CMD_TO_DOF_REORDER] = cmd`, а не `cmd[CMD_TO_DOF_REORDER]`). **Результат: Go2 стабильно стоит под политикой NVIDIA (cb2a68d) и под математическим Rust-контроллером (e6e74f6); эталонная поза [0,0.67,-1.3] из go2_isaac_ros2 поднимает робота на Z=0.36 (проверено через --hold-stance); осталось откалибровать IK контроллера под эту конвенцию углов (проблема 26).** |
