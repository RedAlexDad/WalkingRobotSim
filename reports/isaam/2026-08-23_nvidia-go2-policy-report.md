...
- `src/isaac/isaac_rust_bridge.py` — gains (hip=25, upper/lower=100), расширенные лимиты [-100,100], усиленный лог ([APPLY] targets в градусах, [LEGS] все ноги факт+команда), опции `--hold-stance` и `--calibrate`.
...
| Ходьба (TROT) | не работает | не работает (hip-only паттерн) |
| NaN-взрыв физики | был на URDF | нет на ассете NVIDIA |

**Вывод:** калибровка позы работает — робот стоит на Z=0.36. Ходьба не работает, т.к. контроллер выражает движение hip-качанием, а не подъёмом стоп через calf. Требуется правка ядра контроллера.
...
| 🟡 (1-4ч) | 5 |
| 🔴 (>4ч) | 1 |
| Ключевые выводы | Готовое решение NVIDIA (ассет Mujoco_Menagerie + RL-политика) работает «из коробки», если ассеты скачаны локально и задан asset_root. Multi-physics ассет требует выбора variant Physics. Команды политике передаются torch.Tensor на cuda. При подключении Rust-контроллера к ассету NVIDIA: собственный SingleThreadedExecutor решает смерть контекста rclpy (guard_condition невалиден); ремаппинг DOF требует правильной индексации (`targets[CMD_TO_DOF_REORDER] = cmd`, а не `cmd[CMD_TO_DOF_REORDER]`). **Результат: Go2 стабильно стоит под политикой NVIDIA (cb2a68d) и под математическим Rust-контроллером (e6e74f6); калибровка позы через сдвиг углов (thigh-0.897, calf-1.3) поднимает робота на Z=0.36 (29e1cfa); ходьба требует правки ядра контроллера — его TROT выражает движение hip-качанием, а не подъёмом стоп через calf (проблема 26).** |
