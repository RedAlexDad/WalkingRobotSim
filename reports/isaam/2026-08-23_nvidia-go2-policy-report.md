...
Робот на ассете NVIDIA (через `isaac_rust_bridge.py`) заваливается на бок:
...
Команды применяются (FACT ≈ CMD), робот едет и поворачивается, но roll растёт до 180°.

### 26.2. Гипотезы

- ❌ **Гипотеза A:** знаки суставов перевёрнуты (ассет ≠ URDF). **Опровергнута** — оси совпадают (hip=X, thigh/calf=Y), default stance из env.yaml совпадает с позой ассета.
- ✅ **Гипотеза B:** математический контроллер тюнен под физику URDF/Gazebo, на ассете NVIDIA физика другая → теряет равновесие. **Принята** — робот реально ходит, но кренится.
...
Две причины: (1) неверный ремаппинг DOF (команды в не те суставы) — [проблема 27](#27-проблема-неверный-ремаппинг-dof-команды-в-не-те-суставы); (2) после исправления ремаппинга робот стоит прямо (rpy≈0°), но «сидит» низко (Z≈0.08) — конвенция знаков calf/thigh математики контроллера отличается от ассета NVIDIA.

### 26.4. Диагностика

```
# После исправления ремаппинга (проблема 27):
CMD  FR_thigh_joint=+1.567  FR_calf_joint=-0.000 ...
FACT FR_thigh_joint=+1.574  FR_calf_joint=+0.005 ...   # суставы следуют!
[REPORT] pos=(+0.03,-0.00,+0.08) rpy=(-0.0°,-6.0°,+0.0°)  # прямо, но низко
```

Суставы идеально следуют (joint_err≈0.05), робот стоит прямо, но тело на Z=0.08 — поза «на корточках» (thigh=+1.567, calf=0), а не высокая стойка (thigh=0.8, calf=-1.5).
...
- `src/isaac/isaac_rust_bridge.py` — gains (hip=25, upper/lower=100), расширенные лимиты [-100,100], усиленный лог ([HIP], [FEET], полная CMD в REPORT).
...
| Высота тела Z | — | 0.08 («сидит», не стои́т) |
