...
pos=(-0.51,+0.26,+0.27) rpy=(+2.4°,-0.4°,+58.3°)   # идёт по -X, yaw +58°
pos=(-1.25,+0.01,+0.27) rpy=(+3.9°,-8.7°,+100.9°)  # yaw растёт до +100°

# vy=0.3 (вбок):
pos=(-0.93,+0.79,+0.14) rpy=(-29.6°,-2.3°,+130.4°)  # yaw +130°
```

Робот движется, но крутится вокруг вертикальной оси (yaw непрерывно растёт), а не идёт по заданному направлению.

### 28.2. Гипотезы

- ✅ **Гипотеза A:** hip-качание контроллера создаёт вращение. **Принята** — hip в ассете = абдукция (вбок), качание hip (±0.07 rad) даёт боковое смещение и yaw-момент, а не движение вперёд.
- ❌ **Гипотеза B:** команда скорости неверна. **Опровергнута** — vx=0.3 подаётся корректно, робот реально движется.
- ❌ **Гипотеза C:** перепутаны оси X/Y в конвенции. **Опровергнута как первопричина** — движение вперёд уходит в Y или -X в зависимости от фазы, что указывает на hip-вращение, а не на простую перестановку осей.

### 28.3. Причина

Конвенция движения контроллера не совпадает с ассетом:
- Контроллер выражает движение вперёд через **hip** (в локальной системе ноги), а в ассете NVIDIA hip — это абдукция (вбок), создающая вращение.
- Отсутствует стабилизация курса: контроллер не компенсирует yaw-дрейф.

### 28.4. Диагностика

```
# Диагностика контроллера (Rust FOOT/ANGL) показала правильную стойку
# при vx=0: thg=0.86, clf=-1.88. При vx=0.3 робот движется, но крутится.
# Переключение осей: vx → Y/-X, vy → X — оси перепутаны.
```

### 28.5. Решение

Стабилизация направления в ядре контроллера:
1. Перераспределить движение: вперёд-назад через thigh, вбок через hip (как в ассете), а не hip-only.
2. Добавить стабилизацию курса: компенсировать yaw-дрейф (по IMU), удерживая заданное направление.
3. Возможно, скорректировать оси в `compute_local_positions`/`r_legs`.

### 28.6. Исправление в скриптах/конфигах

- `src/quadropted_controller_rust/quadropted-core/src/controllers/trot/*.rs` — перераспределение движения, стабилизация курса.
- `src/quadropted_controller_rust/quadropted-nodes/src/bin/robot_controller_node.rs` — применение.

### 28.7. Результат

| Метрика | До | После |
|---|---|---|
| Робот ходит (TROT) | да | да |
| Направление | дрейф по Y/-X, yaw растёт | не решено (в работе) |
| Стабилизация курса | нет | не решено (в работе) |
