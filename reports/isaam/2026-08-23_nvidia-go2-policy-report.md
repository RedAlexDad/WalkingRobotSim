...
25. [Проблема: Контекст rclpy умирает после play (guard_condition невалиден)](#25-проблема-контекст-rclpy-умирает-после-play-guard_condition-невалиден)
26. [Проблема: Робот теряет равновесие на ассете NVIDIA](#26-проблема-робот-теряет-равновесие-на-ассете-nvidia)
27. [Проблема: Неверный ремаппинг DOF — команды в не те суставы](#27-проблема-неверный-ремаппинг-dof-команды-в-не-те-суставы)
28. [Проблема: Нестабильное направление движения (дрейф по Y/-X, yaw-вращение)](#28-проблема-нестабильное-направление-движения-дрейф-по-y-x-yaw-вращение)
...
#### A.3.8. Подключение Rust-контроллера к ассету NVIDIA

Создан `isaac_rust_bridge.py` (мост Rust-контроллер → ассет NVIDIA) и `run_rust_bridge.sh` (обёртка окружения). Найдены порядок DOF ассета и ремаппинг из команд контроллера. Робот ходит и поворачивается под математическим контроллером, но теряет равновесие — [проблемы 25](#25-проблема-контекст-rclpy-умирает-после-play-guard_condition-невалиден) и [26](#26-проблема-робот-теряет-равновесие-на-ассете-nvidia).
...
| [25](#25-проблема-контекст-rclpy-умирает-после-play-guard_condition-невалиден) | Контекст rclpy умирает после play | Глобальный executor привязан к старому контексту → guard_condition невалиден | Свой SingleThreadedExecutor в spin_ros | [x] решено |
| [26](#26-проблема-робот-теряет-равновесие-на-ассете-nvidia) | Робот теряет равновесие/сидит | Ремаппинг неверен (27) + конвенция знаков calf/thigh | Тюнинг знаков, robot_height; go2_isaac_ros2 | [ ] не решено |
| [27](#27-проблема-неверный-ремаппинг-dof-команды-в-не-те-суставы) | Неверный ремаппинг DOF | `last_cmd[CMD_TO_DOF_REORDER]` вместо `targets[...] = last_cmd` | Правильная индексация | [x] решено |
| [28](#28-проблема-нестабильное-направление-движения-дрейф-по-y-x-yaw-вращение) | Нестабильное направление | hip-качание создаёт вращение, нет стабилизации курса | Перераспределение движения + стабилизация курса в ядре | [ ] не решено |
...
| 25 | Контекст rclpy умирает после play | (1) Isaac вызывает rcl_shutdown; (2) глобальный executor невалиден; (3) конфликт init | Глобальный executor rclpy создаёт guard_condition от старого контекста; после рестарта контекста timer невалиден | Свой `SingleThreadedExecutor` в spin_ros (создаётся от текущего контекста) | чтение rclpy/__init__.py, лог spin_once | 🟡 |
| 26 | Робот теряет равновесие/сидит на ассете NVIDIA | (1) жёсткий hip; (2) IMU-компенсация; (3) ремаппинг; (4) знаки углов | Ремаппинг DOF неверен (проблема 27); после исправления — конвенция знаков calf/thigh отличается от ассета | Тюнинг знаков calf/thigh, robot_height; изучение go2_isaac_ros2 | усиленный лог (HIP/FEET/CMD) | 🔴 |
| 27 | Неверный ремаппинг DOF | (1) индексация перепутана; (2) знаки неверны | `last_cmd[CMD_TO_DOF_REORDER]` вместо `targets[CMD_TO_DOF_REORDER] = last_cmd`; факт читался через инверсию | Правильная индексация в apply_pending_command и чтении факта | лог HIP (cmd=+1.566 при hip=0) | 🟡 |
| 28 | Нестабильное направление | (1) hip-качание даёт вращение; (2) оси X/Y перепутаны; (3) нет стабилизации курса | Конвенция движения контроллера не совпадает с ассетом: hip в ассете = абдукция (вбок) | Перераспределить движение (вперёд через thigh, вбок через hip) + стабилизация курса | лог REPORT (yaw растёт), подача vx/vy | 🔴 |
...
## 25. Проблема: Контекст rclpy умирает после play (guard_condition невалиден)

### 25.1. Симптом

Мост `isaac_rust_bridge.py` подписан на команды Rust-контроллера, но через ~2 секунды после `timeline.play()` поток rclpy умирает:

```
spin_once error: failed to create timer: the given context is not valid, either
rcl_init() was not called or rcl_shutdown() was called., at
/workspace/jazzy_ws/src/rcl/rcl/src/rcl/guard_condition.c:67 — поток rclpy УМИРАЕТ
```

Счётчик `cmd=` замирает (например 100 или 125), хотя контроллер продолжает публиковать.

### 25.2. Гипотезы

- ❌ **Гипотеза A:** Isaac Sim вызывает `rcl_shutdown()`. **Опровергнута** — `rclpy.ok()` остаётся `True`, узел жив для публикации (`js=` растёт), умирает только spin.
- ✅ **Гипотеза B:** глобальный executor rclpy привязан к старому контексту → guard_condition невалиден. **Принята** — `rclpy.spin_once()` использует глобальный executor, созданный при первом вызове; его guard_condition привязан к контексту, который был пересоздан.

### 25.3. Причина

`rclpy.spin_once(node)` (без аргумента executor) использует `get_global_executor()`. Глобальный executor создаётся один раз и привязывается к контексту. Если контекст rclpy пересоздаётся (Isaac Sim при инициализации PhysX/play пересоздаёт rclpy-контекст), то guard_condition глобального executor остаётся привязан к старому (закрытому) контексту → `failed to create timer: the given context is not valid`.

### 25.4. Диагностика

```
# rclpy/__init__.py
def spin_once(node, *, executor=None, ...):
    executor = get_global_executor() if executor is None else executor   # глобальный!
    executor.add_node(node)
    executor.spin_once(...)

# get_global_executor() создаёт SingleThreadedExecutor один раз и
# привязывает его к get_default_context(); при рестарте контекста
# executor не пересоздаётся.
```

### 25.5. Решение

Создать **собственный** `SingleThreadedExecutor` внутри `spin_ros` от текущего контекста, пересоздавать его при ошибке, и добавить автовосстановление rclpy (`rclpy.shutdown()` + `rclpy.init()` + пересоздание узла и pub/sub) в случае смерти контекста:
...
### 25.6. Исправление в скриптах/конфигах

- `src/isaac/isaac_rust_bridge.py` — `spin_ros`: собственный executor + автовосстановление rclpy (`_recreate_pubsub`).

### 25.7. Результат

| Метрика | До | После |
|---|---|---|
| Поток rclpy | умирал через ~2 с | стабилен (spin=OK) |
| `cmd=` (команды) | замирал на ~100-125 | растёт непрерывно (3000+) |
| Публикация joint_states | замирала | растёт (js=2800+) |

**Связь с развёртыванием (Часть A):** встречена на этапе [A.3.8 «Подключение Rust-контроллера к ассету NVIDIA»](#a38-подключение-rust-контроллера-к-ассету-nvidia).

---

## 26. Проблема: Робот теряет равновесие на ассете NVIDIA

### 26.1. Симптом

Робот на ассете NVIDIA (через `isaac_rust_bridge.py`) сначала заваливается на бок:

```
[REPORT] pos=(-1.36,-1.10,+0.31) rpy=(+43.5°,-8.6°,-74.2°)   # кренится
[REPORT] pos=(-2.21,-0.94,+0.08) rpy=(+177.3°,-7.2°,-56.2°)  # перевернулся
```

После исправления ремаппинга (проблема 27) робот стоит прямо (rpy≈0°), но **«сидит» на корточках**: высота тела Z≈0.08 вместо ~0.3 (правильная стойка).

```
[REPORT] pos=(+0.03,-0.00,+0.08) rpy=(-0.0°,-6.0°,+0.0°)  # прямо, но низко (сидит)
CMD  FR_thigh_joint=+1.567  FR_calf_joint=-0.000 ...      # thigh согнут, calf выпрямлен
```

### 26.2. Гипотезы

- ❌ **Гипотеза A:** знаки суставов перевёрнуты (ассет ≠ URDF). **Опровергнута** — оси совпадают (hip=X, thigh/calf=Y).
- ✅ **Гипотеза B:** математический контроллер тюнен под физику URDF/Gazebo, конвенция углов не совпадает с ассетом NVIDIA. **Принята** — контроллер шлёт thigh=1.567, calf=0, а ассет требует thigh=0.67, calf=-1.3.
- ❌ **Гипотеза C:** hip жёсткий (stiffness=100) мешает балансировке. **Опровергнута** — подобраны gains hip=25 (как у политики NVIDIA).
- ✅ **Гипотеза D:** ремаппинг DOF неверен → команды попадают не в те суставы. **Принята** — см. [проблему 27](#27-проблема-неверный-ремаппинг-dof-команды-в-не-те-суставы).

### 26.3. Причина

Три последовательные причины:
1. **Ремаппинг DOF неверен** — команды hip/thigh/calf попадали не в те суставы (см. [проблему 27](#27-проблема-неверный-ремаппинг-dof-команды-в-не-те-суставы)). После исправления робот стоит прямо.
2. **Конвенция углов не совпадает с ассетом** — IK контроллера `compute_joint_angles_for_leg` выдаёт для стойки `thigh=+1.567, calf=0` (нога почти выпрямлена), а ассет NVIDIA требует `thigh=0.67, calf=-1.3` (ноги согнуты «жабкой»).
3. **Знак и масштаб calf/thigh в IK контроллера не соответствуют ассету.**

### 26.4. Диагностика

```
# Суставы следуют командам контроллера (joint_err≈0.05) — проблема не в следовании,
# а в САМИХ командах:
CMD  FR_thigh_joint=+1.567  FR_calf_joint=-0.000 ...      # контроллер «сажает» робота
FACT FR_thigh_joint=+1.574  FR_calf_joint=+0.005 ...      # суставы точно следуют

# Робот стоит прямо, но низко:
[REPORT] pos=(+0.03,-0.00,+0.08) rpy=(-0.0°,-6.0°,+0.0°)
```

**Проверка эталонной позы (решающий эксперимент):** в мост добавлена опция `--hold-stance`, которая держит правильную позу `[hip=0, thigh=0.67, calf=-1.3]`, игнорируя команды контроллера:

```
[REPORT] pos=(-0.01,+0.00,+0.36) rpy=(-0.0°,-0.0°,+0.1°)  # РОБОТ ВСТАЛ, Z=0.36!
```

Результат: с правильной позой робот стабильно стоит на **Z=0.36** (было 0.08). Это подтверждает, что ассет и поза корректны, а проблема — в командах контроллера.

**Анализ IK контроллера** (`compute_joint_angles_for_leg`):

```
theta4 = -sqrt(1 - d²)·atan2(d)     # calf
  при d→1 (нога выпрямлена): theta4 → 0   ← контроллер шлёт calf=0
  при d<0 (нога согнута):    theta4 > 0   ← но ассет требует calf<0 (отрицательный!)
theta3 = atan2(z, g) - atan2(l4·sin(theta4), l3 + l4·cos(theta4))   # thigh
```

Правильная стойка `calf=-1.3` означает сильный сгиб голени, а IK контроллера даёт `calf=0` (выпрямление). Нужна калибровка конвенции calf/thigh.
...
**Реализовано — сдвиг углов в мосте (`--calibrate`):**

Контроллер шлёт `thigh≈+1.57 (90°), calf=0` — поза «ноги вбок». Эталон ассета: `thigh=0.67, calf=-1.3`. В `apply_pending_command` добавлен сдвиг в командном порядке (FR,FL,RR,RL × hip,thigh,calf):

```
thigh' = thigh - 0.897   # 1.567 → 0.67 (38°)
calf'  = calf  - 1.3     # 0 → -1.3 (-74°)
```

**Результат калибровки: робот встаёт на Z=0.36** (проверено через `--calibrate` и `--hold-stance`).

**Диагноз ходьбы:** в TROT контроллер выражает движение только hip-качанием (±0.07 rad = ±4°), а `calf/thigh` остаются постоянными (подтверждено логом `[APPLY]`). Свинг-подъём стоп в его конвенции даёт hip-вращение, а не сгиб calf. Для ходьбы нужно переписать генерацию походки в ядре контроллера (свинг через calf/thigh).

Идея интеграции с IsaacLab сохранена в [A.6 «Дальнейшие шаги»](#a6-дальнейшие-шаги).

### 26.6. Исправление в скриптах/конфигах

- `src/isaac/isaac_rust_bridge.py` — gains (hip=25, upper/lower=100), расширенные лимиты [-100,100], усиленный лог ([APPLY] targets в градусах, [LEGS] все ноги факт+команда), опции `--hold-stance` и `--calibrate`, спавн робота на z=0.6 (не проваливается в TROT).
- `src/quadropted_controller_rust/quadropted-nodes/src/bin/robot_controller_node.rs` — диагностика `[Rust FOOT]`/`[Rust ANGL]` (позиции стоп и углы) каждые 120 тиков.

### 26.7. Результат

| Метрика | До | После |
|---|---|---|
| Робот стоит прямо (rpy) | 180° (перевёрнут) | ≈0° |
| Суставы следуют CMD | частично | joint_err≈0.05 |
| Высота тела с командами контроллера | 0.08 («сидит») | 0.36 (**стоит**, через --calibrate) |
| Калибровка позы | — | thigh 90°→38°, calf 0°→-74° |
| Ходьба (TROT) | не работает | **работает** (vx=0.3, робот движется) |
| Свинг стоп в TROT | hip-only (±4°) | calf меняется (-1.7/-1.88) — подъём стоп |
| Направление движения | — | **нестабильно** (Y/-X, yaw дрейф) |
| NaN-взрыв физики | был на URDF | нет на ассете NVIDIA |

**Вывод:** калибровка позы и ходьба работают — робот ходит в TROT на ассете NVIDIA (Z≈0.27, не проваливается). Осталось стабилизировать направление: робот дрейфует по Y/-X и крутится (yaw растёт), т.к. hip-качание контроллера создаёт вращение. Требуется стабилизация курса в ядре контроллера.

**Связь с развёртыванием (Часть A):** встречена на этапе [A.3.8 «Подключение Rust-контроллера к ассету NVIDIA»](#a38-подключение-rust-контроллера-к-ассету-nvidia).

---

## 27. Проблема: Неверный ремаппинг DOF — команды в не те суставы

### 27.1. Симптом

После подключения Rust-контроллера к ассету NVIDIA робот заваливался/кренился. Усиленный лог показал аномалию: при команде hip=0 факт hip=+1.595, а thigh не следовал команде.

```
[HIP] cmd=[+1.566 -0.000 +0.000 +0.000] fact=[+1.582 -1.175 +1.269 +0.105] (FL FR RL RR)
# cmd для FL hip = +1.566 при том, что контроллер шлёт hip=0 — неверный порядок
```

### 27.2. Гипотезы

- ✅ **Гипотеза A:** индексация ремаппинга перепутана. **Принята** — `last_cmd[CMD_TO_DOF_REORDER]` брал не те элементы; правильная операция `targets[CMD_TO_DOF_REORDER] = last_cmd`.
- ❌ **Гипотеза B:** знаки суставов неверны. **Опровергнута** — после исправления ремаппинга робот встал прямо.

### 27.3. Причина

`apply_pending_command` использовал `cmd = last_cmd[CMD_TO_DOF_REORDER]` (обратная индексация), а не `targets[CMD_TO_DOF_REORDER] = last_cmd`. В результате команды hip/thigh/calf попадали не в те DOF: hip получал значение thigh и наоборот.

### 27.4. Диагностика

```
# Неверно (брало элементы по индексам):
cmd = self.last_cmd[CMD_TO_DOF_REORDER].copy()
# Правильно (присваивание по индексам):
targets = np.zeros(12); targets[CMD_TO_DOF_REORDER] = self.last_cmd
```

Аналогичная ошибка была в чтении факта (использовалась инверсия `DOF_TO_CMD_REORDER`, а нужна прямая `positions[CMD_TO_DOF_REORDER]`).

### 27.5. Решение

- `apply_pending_command`: `targets[CMD_TO_DOF_REORDER] = self.last_cmd`.
- Чтение факта в командном порядке: `dof_pos[CMD_TO_DOF_REORDER]` (прямая индексация, без инверсии).

### 27.6. Исправление в скриптах/конфигах

- `src/isaac/isaac_rust_bridge.py` — `apply_pending_command`, публикация joint_states, диагностика.

### 27.7. Результат

| Метрика | До | После |
|---|---|---|
| hip факт при cmd=0 | +1.595 (неверно) | ±0.31 (симметрично) |
| thigh следует CMD | нет | да (joint_err≈0.05) |
| Поза робота | кренится/перевёрнут | стоит прямо (rpy≈0°) |

**Связь с развёртыванием (Часть A):** встречена на этапе [A.3.8 «Подключение Rust-контроллера к ассету NVIDIA»](#a38-подключение-rust-контроллера-к-ассету-nvidia).

---

## 28. Проблема: Нестабильное направление движения (дрейф по Y/-X, yaw-вращение)

### 28.1. Симптом

Робот в TROT ходит (vx=0.3), но направление нестабильно:

```
# vx=0.3 (должен идти вперёд по +X):
pos=(+0.43,+0.13,+0.37) rpy=(-19.1°,-39.1°,+156.1°)  # идёт по X, но кренится
pos=(+0.73,+0.25,+0.07) rpy=(-130.9°,-0.1°,-73.8°)   # переворачивается (roll -131°)

# С clamp hip ±0.15:
pos=(+1.99,-0.64,+0.15) rpy=(-69.6°,+0.0°,+174.4°)   # идёт по X, но кренится
```

Робот **движется по X** (вперёд!), но заваливается на бок (roll→180°) — падает.

### 28.2. Гипотезы

- ✅ **Гипотеза A:** hip-амплитуды контроллера слишком большие. **Принята** — контроллер в TROT генерирует hip до ±1.5 (97°), что в 20 раз больше нормы (±0.07). Clamp hip ±0.3/±0.15 в мосте ограничивает, но робот всё равно падает.
- ❌ **Гипотеза B:** команда скорости неверна. **Опровергнута** — vx=0.3 подаётся корректно, робот реально движется по X.
- ❌ **Гипотеза C:** перепутаны оси X/Y. **Опровергнута** — движение вперёд идёт по X (локальный z → thigh, подтверждено диагностикой LOCL), обмен X/Y в swing/stance не помог.

### 28.3. Причина

Математический контроллер TrotGait тюнен под **Gazebo-физику**, где его большие hip-амплитуды компенсировались иначе. На ассете NVIDIA (правильные массы/инерции/трение) те же команды валят робота на бок. Диагностика LOCL показала: локальные координаты стоп в TROT достигают x=±0.5 (стопы уходят далеко вбок), что даёт hip-амплитуды ±1.5. ПД-регулятор не успевает удержать баланс.

### 28.4. Диагностика

```
# Диагностика контроллера (Rust FOOT/ANGL/LOCL):
[Rust LOCL] x=[0.578 0.453 0.538 0.514] ...   # стопы вбок до 0.5
[Rust ANGL] hip=[-1.534 -1.543 -1.323 -1.783]  # hip до ±1.5 (97°)

# Контакты диагональных пар корректны (FR-RL / FL-RR), трот-паттерн верный.
# Проблема — амплитуда hip, а не паттерн.
```

### 28.5. Решение

**Решение принято: переход на IsaacLab (go2_isaac_ros2).** Калибровка нашего TrotGait под Isaac требует много итераций (конвенция и физика отличаются от Gazebo). В go2_isaac_ros2 ходьба уже работает: правильные углы (STANDING_JOINT_ANGLES=[0,0.67,-1.3], PRONE), ПД stiffness=75/damping=0.5, канал `/lowcmd` для внешнего контроллера. Наш Rust-контроллер может публиковать `/lowcmd` (углы + kp/kd).

Идея IsaacLab сохранена в [A.6 «Дальнейшие шаги»](#a6-дальнейшие-шаги).

### 28.6. Исправление в скриптах/конфигах

- `src/isaac/isaac_rust_bridge.py` — clamp hip ±0.3→±0.15 (временная мера).
- `src/quadropted_controller_rust/quadropted-nodes/src/bin/robot_controller_node.rs` — IMU-yaw подписка, yaw-компенсация (отключена — вызывает крен), диагностика LOCL/CONT.
- Решение: установка IsaacLab + go2_isaac_ros2.

### 28.7. Результат

| Метрика | До | После |
|---|---|---|
| Робот ходит (TROT) | да | да (по X) |
| Направление | дрейф, падает | **не решено — переход на IsaacLab** |
| Стабилизация | — | не решено |
| Диагностика | — | LOCL/CONT/ANGL в контроллере |

**Вывод:** TrotGait тюнен под Gazebo, для Isaac требуется IsaacLab. Робот на ассете NVIDIA стоит и ходит по X, но нестабилен в TROT из-за hip-амплитуд.
