...
5. [Текущие 31 проверка](#5-текущие-31-проверка)
...
Разработан и закоммичен интеграционный тест-скрипт `scripts/test_sim_integration.sh` (31 проверка), который тестирует **всю систему целиком** против живой Gazebo-симуляции с Rust-контроллером: от спавна робота до SLAM-картографии и движения. В ходе разработки выявлено и устранено **3 реальные проблемы**:
...
graph LR
    ...
    subgraph "Уровень 4: Живая симуляция (НОВОЕ)"
        U4[31 проверка<br>узлы, топики, TF,<br>SLAM, движение]
    end
```
...
graph LR
    ...
    D --> E[Блок 12-13<br>Движение + остановка]
    E --> F[Блок 14<br>Сервисы]
```
...
Все 31 проверка прошли на чистой симуляции:
...
1. **Интеграционный тест-скрипт** `scripts/test_sim_integration.sh` — 31 проверка против живой симуляции
...
1. **Реализовать 10 дополнительных проверок** (приоритет: частоты, EKF-консистентность, рост карты, foot_contact)
...
| Файл | Действие |
|------|----------|
| `scripts/test_sim_integration.sh` | **Новый** — интеграционные тесты (31 проверка) |
