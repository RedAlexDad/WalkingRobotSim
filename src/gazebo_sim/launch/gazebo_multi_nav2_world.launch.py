...
def generate_launch_description():
    ...
    declare_camera_fps = DeclareLaunchArgument(
        name="camera_fps", default_value="30", description="Camera update rate (FPS)"
    )
    ...
    map_server = Node(
        ...
        parameters=[
            {
                "yaml_filename": os.path.join(pkg_path, "maps", "cambridge.yaml"),
            },
        ],
        ...
        if last_action is None:
            ...
            spawn_robot_event = RegisterEventHandler(
                event_handler=OnProcessExit(
                    target_action=last_action, on_exit=[robot_group]
                )
            )
