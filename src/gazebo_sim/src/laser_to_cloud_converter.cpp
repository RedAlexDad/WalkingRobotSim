...
void cb(const gz::msgs::LaserScan &msg, gz::transport::Node::Publisher &pub) {
    ...
    double v_angle_step = (v_count > 1) ? (v_angle_max - v_angle_min) / (v_count - 1) : 0.0;
...
int main(int, char **) {
    ...
    std::function<void(const gz::msgs::LaserScan &)> subCb = [pub](const gz::msgs::LaserScan &msg) mutable {
        cb(msg, pub);
    };
