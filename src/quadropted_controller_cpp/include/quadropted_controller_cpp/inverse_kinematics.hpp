...
/// Вычисление локальных позиций точек опор
inline Eigen::MatrixXd compute_local_positions(
...
/// Углы суставов для одной ноги с защитой от math domain error
inline std::array<double, 3> compute_joint_angles_for_leg(
...
public:
    ...
    Eigen::MatrixXd get_local_positions(const Eigen::MatrixXd& leg_positions,
                                         double dx, double dy, double dz,
    ...
    std::vector<double> inverse_kinematics(const Eigen::MatrixXd& leg_positions,
                                            double dx, double dy, double dz,
                                            double roll, double pitch, double yaw) const {
