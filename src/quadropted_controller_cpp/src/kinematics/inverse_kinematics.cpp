    ...
    double l1, double l2, double l3, double l4)
{
    ...
    double theta3 = std::atan2(z, G) - std::atan2(
        l4 * std::sin(theta4), l3 + l4 * std::cos(theta4));
...
Eigen::MatrixXd InverseKinematics::get_local_positions(
    const Eigen::MatrixXd& leg_positions,
    double dx, double dy, double dz,
...
std::vector<double> InverseKinematics::inverse_kinematics(
    ...
    double roll, double pitch, double yaw) const
{
    Eigen::MatrixXd positions = get_local_positions(leg_positions, dx, dy, dz,
                                                      roll, pitch, yaw);
