...
WaypointTool::WaypointTool() {
    topic_property_ = new rviz_common::properties::StringProperty("Topic", "/custom_goal_pose",
...
void WaypointTool::onInitialize() {
    ...
    pose_publisher_ = node->create_publisher<geometry_msgs::msg::PoseStamped>(topic_property_->getStdString(), 10);
}
...
int WaypointTool::processMouseEvent(rviz_common::ViewportMouseEvent& event) {
    if (event.leftDown()) {
        ...
        Ogre::Ray mouse_ray =
            camera->getCameraToViewportRay(static_cast<Ogre::Real>(event.x) / static_cast<Ogre::Real>(width),
        ...
        if (intersection.first) {
            ...
            RCLCPP_INFO(context_->getRosNodeAbstraction().lock()->get_raw_node()->get_logger(),
                        "Published waypoint: x=%f, y=%f", point.x, point.y);
        }
        return Finished;
    }
    return 0;
