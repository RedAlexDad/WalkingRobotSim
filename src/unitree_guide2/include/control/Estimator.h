...
/**********************************************************************
Версия 0.1(250313)
Описание: Оценка состояния шагающего робота с использованием данных датчиков и методов, подобных фильтру Калмана, таких как определение класса, положение стопы, скорость, информация об осанке и т.д.
Функции
- Оценщик: принимает модель робота, данные датчиков, контактную информацию моста и т.д. и инициализирует внутреннее состояние, dt= интервал выборки
- GetPosition: возвращает предполагаемое положение робота, оценивает положение центра робота (CoM) путем объединения данных датчика и информации о положении ног.
- getVelocity: Возвращает расчетную скорость робота
- getFootPos: Возвращает текущее местоположение определенного моста
- getFeetVel: Возвращает скорость всех ног
- getPosFeet2BGlobal: Преобразует положение ноги из системы координат тела в глобальную систему координат
- _initSystem: функция настройки исходного состояния
***********************************************************************/
#ifndef ESTIMATOR_H
#define ESTIMATOR_H
...
private:
    ...
    RotMat _robRotMat; // Rotate Matrix: from body to global
    Vec3 _g;
