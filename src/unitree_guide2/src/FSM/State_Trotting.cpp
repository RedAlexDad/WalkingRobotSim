      ...
      _balCtrl(ctrlComp->balCtrl)
{
    ...
    // Этот параметр который необходимо откаблировать
    _Kpp = Vec3(70, 70, 90).asDiagonal(); // 150 150 200
...
void State_Trotting::run()
{
    ...
    _RobRotMat = _lowState->getRotMat();
    _G2BRotMat = _RobRotMat.transpose();
...
{
    /* Movement */
    _vCmdGlobal = _RobRotMat * _vCmdBody;
...
void State_Trotting::calcTau()
{
    ...
    _dWbd = _kpw * rotMatToExp(_Rd * _G2BRotMat) + _Kdw * (_wCmdGlobal - _lowState->getGyroGlobal());
    ...
    _forceFeetGlobal = -_balCtrl->calF(_ddPcd, _dWbd, _RobRotMat, _posFeet2BGlobal, *_contact);
    ...
    _forceFeetBody = _G2BRotMat * _forceFeetGlobal;
...
void State_Trotting::calcQQd()
{
    ...
    for (int i(0); i < 4; ++i)
    {
        _posFeet2BGoal.col(i) = _G2BRotMat * (_posFeetGlobalGoal.col(i) - _posBody);
        _velFeet2BGoal.col(i) = _G2BRotMat * (_velFeetGlobalGoal.col(i) - _velBody);
        // _velFeet2BGoal.col(i) = _G2BRotMat * (_velFeetGlobalGoal.col(i) - _velBody - _RobRotMat * (skew(_lowState->getGyro()) * _posFeet2B.col(i)) );  //  c.f formula (6.12)
    }
