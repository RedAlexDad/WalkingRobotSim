...
Go2Robot::Go2Robot(){
    ...
    // pHip2B представляет собой вектор, идущий от центра фюзеляжа к началу системы координат основания опоры.
    ...
    // Представляет координаты нейтральной точки опоры каждой ноги в системе координат фюзеляжа {b}.
    // _feetPosNormalStand <<  0.1881,  0.1881, -0.1881, -0.1881,
    ...
    _robVelLimitX << -0.4, 0.4; // Интервал скорости перемещения робота в направлении оси x системы координат фюзеляжа {b}
    _robVelLimitY << -0.3, 0.3; // Интервал скорости перемещения робота в направлении оси y системы координат фюзеляжа {b}
    _robVelLimitYaw << -0.5, 0.5; //Интервал скорости перемещения робота в направлении оси z системы координат фюзеляжа {b}


#ifdef COMPILE_WITH_REAL_ROBOT
    _mass = 6.921 + 0.678 * 4 + 1.152 * 4 + 0.154 * 4; // 14.857 kg
    _pcb << 0.0, 0.0, 0.0; //_pcd: Заданное положение фюзеляжа в мировой системе координат.
    _Ib = Vec3(0.0792, 0.2085, 0.2265).asDiagonal();
/* Тензор инерции робота, поскольку мы упрощаем робота в целом, превращая его в твердое тело，
    Таким образом, этот тензор инерции должен соответствовать тензору инерции робота, когда он примерно стоит.*/
#endif  // COMPILE_WITH_REAL_ROBOT
